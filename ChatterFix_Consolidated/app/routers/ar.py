...
async def ar_work_orders(request: Request):
    ...
    work_orders = conn.execute("SELECT * FROM work_orders WHERE status != 'Completed' ORDER BY priority DESC").fetchall()
    conn.close()
    return templates.TemplateResponse("ar/work_orders.html", {"request": request, "work_orders": work_orders})
...
async def ar_work_order_detail(request: Request, wo_id: int):
    ...
    return templates.TemplateResponse("ar/work_order_detail.html", {"request": request, "wo": wo})
