...
async def inventory_list(request: Request):
    ...
    """).fetchall()
    conn.close()
    return templates.TemplateResponse("inventory/list.html", {"request": request, "parts": parts})
...
async def add_part_form(request: Request):
    ...
    conn.close()
    return templates.TemplateResponse("inventory/add.html", {"request": request, "vendors": vendors})
...
async def add_part(
    ...
    try:
        cursor = conn.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (name, part_number, category, description, current_stock, minimum_stock, location, unit_cost, vendor_id))
            ...
            for file in files:
                if file.filename:
                    ...
                    if file_type == "image":
                        ...
                        current_image = conn.execute("SELECT image_url FROM parts WHERE id = ?", (part_id,)).fetchone()[0]
                        if not current_image:
                            conn.execute("UPDATE parts SET image_url = ? WHERE id = ?", (rel_path, part_id))
...
async def part_detail(request: Request, part_id: int):
    ...
    media = conn.execute("SELECT * FROM part_media WHERE part_id = ? ORDER BY uploaded_date DESC", (part_id,)).fetchall()
    conn.close()
        
    return templates.TemplateResponse("inventory/detail.html", {"request": request, "part": part, "media": media})
...
async def vendor_list(request: Request):
    ...
    vendors = conn.execute("SELECT * FROM vendors ORDER BY name").fetchall()
    conn.close()
    return templates.TemplateResponse("inventory/vendors.html", {"request": request, "vendors": vendors})
...
async def add_vendor(
    ...
    """Add a new vendor"""
    conn = get_db_connection()
    conn.execute("INSERT INTO vendors (name, contact_name, email, phone) VALUES (?, ?, ?, ?)", 
