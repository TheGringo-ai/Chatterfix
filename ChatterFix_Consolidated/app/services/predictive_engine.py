...
try:
    ...
    ML_AVAILABLE = False
    logger.warning("⚠️ ML libraries not available. Install scikit-learn for predictive features.")
...
class PredictiveMaintenanceEngine:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.warning("⚠️ AI orchestrator not available - using basic predictions only")
    ...
    async def initialize_ml_models(self):
        ...
        try:
            ...
            self.feature_columns = [
                'asset_age_days', 'maintenance_frequency', 'last_maintenance_days',
                'total_work_orders', 'avg_repair_time', 'cost_trend',
                'vibration_avg', 'temperature_avg', 'pressure_avg',
                'runtime_hours', 'cycles_count', 'load_factor'
            ]
    ...
    async def load_historical_data(self):
        ...
        try:
            ...
            logger.info(f"📊 Loaded historical data for {len(self.asset_history)} assets")
    ...
    def generate_synthetic_asset_data(self) -> Dict[str, Dict]:
        ...
        asset_types = [
            ("Pump #1", "pump"), ("Generator #1", "generator"), 
            ("HVAC Unit", "hvac"), ("Conveyor Belt", "conveyor"),
            ("Compressor #2", "compressor"), ("Motor #3", "motor")
        ]
        ...
        for i, (name, asset_type) in enumerate(asset_types):
            ...
            for j, date in enumerate(dates):
                ...
                temp = base_temp + seasonal_factor * 10 + degradation * 5 + np.random.normal(0, 2)
                vibration = base_vibration + degradation * 0.1 + np.random.normal(0, 0.05)
                ...
                if np.random.random() < 0.05:  # 5% chance daily
                    work_orders.append({
                        ...
                        'type': np.random.choice(['reactive', 'preventive'], p=[0.7, 0.3]),
    ...
    def extract_features(self, asset_data: Dict) -> List[float]:
        ...
        try:
            ...
            if asset_data['maintenance_history']:
                last_maintenance = max(asset_data['maintenance_history'], 
                                     key=lambda x: x['date'])
            ...
            if total_work_orders > 0:
                avg_repair_time = np.mean([wo['resolution_time'] for wo in asset_data['work_orders']])
            ...
            if recent_sensor_data:
                ...
                temperature_avg = np.mean([s['temperature'] for s in recent_sensor_data])
                pressure_avg = np.mean([s['pressure'] for s in recent_sensor_data])
                runtime_hours = np.mean([s['runtime_hours'] for s in recent_sensor_data])
    ...
    def calculate_current_health_score(self, asset_data: Dict) -> float:
        ...
        try:
            ...
            recent_work_orders = [wo for wo in asset_data['work_orders'] 
                                if datetime.fromisoformat(wo['date']) > datetime.now() - timedelta(days=90)]
    ...
    async def predict_asset_health(self, asset_id: str) -> AssetHealthScore:
        ...
        try:
            ...
            if ai_enhanced_analysis and ai_enhanced_analysis.get("adjusted_health_score"):
                ...
                logger.info(f"AI adjusted health score for {asset_id}: {original_score:.2f} → {health_score:.2f}")
            ...
            if health_score < 0.5:
                if ai_enhanced_analysis and ai_enhanced_analysis.get("predicted_failure_days"):
...
{{
    ...
    "maintenance_urgency": "medium"
            ...
            if ai_response and ai_response.get("response"):
                ...
                try:
                    ...
                    logger.warning(f"⚠️ Failed to parse AI analysis JSON for {asset_id}: {e}")
    ...
    def _format_sensor_data_for_ai(self, sensor_data: List[Dict]) -> str:
        ...
        for i, data in enumerate(sensor_data[-7:]):  # Last 7 days
            ...
            formatted.append(f"  {date}: Temp={temp}°F, Vibration={vibration}, Pressure={pressure}psi")
        
        return '\n'.join(formatted)
    
    def _format_maintenance_history_for_ai(self, maintenance_history: List[Dict]) -> str:
    ...
    def _format_work_order_history_for_ai(self, work_orders: List[Dict]) -> str:
        ...
        for wo in work_orders[-10:]:  # Last 10 work orders
            ...
            formatted.append(f"  {date}: {priority} priority {wotype}, resolved in {resolution_time}h")
...
Focus on practical actions a maintenance technician can implement."""
    ...
    def _extract_recommendations_from_text(self, response_text: str) -> List[str]:
        ...
        for line in lines:
            ...
            if line.startswith(('- ', '• ', '* ')) or (line[:2].replace('.', '').isdigit() and '. ' in line):
    ...
    def identify_contributing_factors(self, asset_data: Dict, features: List[float]) -> List[str]:
    ...
    def generate_maintenance_recommendations(self, asset_data: Dict, health_score: float, risk_level: str) -> List[str]:
        ...
        try:
            ...
            schedule = {
                ...
                "next_month": [
                    {
                        ...
                        "asset_name": asset.asset_name,
                        "health_score": asset.health_score,
                        "recommended_date": (datetime.now() + timedelta(days=21)).strftime("%Y-%m-%d"),
                        "priority": "medium",
                ...
                "summary": {
                    ...
                    "estimated_total_hours": len(critical_assets) * 6 + len(high_risk_assets) * 3 + len(medium_risk_assets) * 2,
    ...
    try:
        ...
        dashboard = {
            "overview": {
                ...
                "assets_at_risk": risk_distribution["critical"] + risk_distribution["high"],
                "maintenance_alerts": schedule["summary"]["critical_count"] + schedule["summary"]["high_risk_count"]
            },
