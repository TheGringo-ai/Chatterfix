...
# Realistic data sets
FIRST_NAMES = ["John", "Sarah", "Mike", "Lisa", "David", "Emily", "James", "Maria", "Robert", "Jennifer", 
               "William", "Linda", "Richard", "Patricia", "Thomas", "Nancy", "Daniel", "Karen", "Matthew", "Betty"]

LAST_NAMES = ["Smith", "Johnson", "Williams", "Brown", "Jones", "Garcia", "Miller", "Davis", "Rodriguez", "Martinez",
              "Hernandez", "Lopez", "Gonzalez", "Wilson", "Anderson", "Thomas", "Taylor", "Moore", "Jackson", "Martin"]

ASSET_TYPES = [
    {"category": "Pumps", "names": ["Centrifugal Pump", "Hydraulic Pump", "Vacuum Pump", "Booster Pump"]},
    {"category": "Motors", "names": ["AC Motor", "DC Motor", "Servo Motor", "Stepper Motor"]},
    {"category": "Conveyors", "names": ["Belt Conveyor", "Roller Conveyor", "Chain Conveyor", "Screw Conveyor"]},
    {"category": "HVAC", "names": ["Air Handler", "Chiller", "Cooling Tower", "Boiler"]},
    {"category": "Compressors", "names": ["Air Compressor", "Gas Compressor", "Refrigeration Compressor"]},
]

LOCATIONS = ["Building A - Floor 1", "Building A - Floor 2", "Building B - Warehouse", "Building C - Production",
             "Outdoor - North Yard", "Outdoor - South Yard", "Maintenance Shop", "Loading Dock"]

PART_NAMES = [
    "Hydraulic Seal", "Motor Bearing", "V-Belt", "Oil Filter", "Air Filter", "Control Panel",
    "Pressure Gauge", "Temperature Sensor", "Relay Switch", "Circuit Breaker", "Coupling",
    "Shaft Seal", "Gasket Kit", "Lubricant Oil", "Coolant", "Grease", "Bolt Set", "Washer Set"
]
...
def clear_existing_data():
    ...
    tables = [
        'user_training', 'training_modules', 'work_order_feedback', 'team_messages',
        'notifications', 'parts_requests', 'user_location_history', 'user_privacy_settings',
        'user_dashboard_config', 'work_orders', 'parts', 'assets', 
        'user_performance', 'user_skills', 'users'
    ]
...
def create_users(count=20):
    ...
    roles = ['technician'] * 12 + ['supervisor'] * 4 + ['parts_manager'] * 2 + ['manager'] * 2
    ...
    for i in range(count):
        ...
        cur.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (username, email, f"{first_name} {last_name}", role, 'active', 'grid', 'dark'))
...
def create_assets(count=50):
    ...
    for i in range(count):
        ...
        install_date = (datetime.now() - timedelta(days=random.randint(365, 3650))).strftime('%Y-%m-%d')
        purchase_date = (datetime.now() - timedelta(days=random.randint(400, 3700))).strftime('%Y-%m-%d')
        ...
        next_service = (datetime.now() + timedelta(days=random.randint(30, 180))).strftime('%Y-%m-%d')
        ...
        """, (
            ...
            random.choice(["Acme Corp", "TechPro", "IndustrialMax", "PowerSystems", "GlobalEquip"]),
            location,
            'Active',
            criticality,
            purchase_date,
...
def create_team_messages(users, count=50):
    ...
    for i in range(count):
        ...
        cur.execute("""
            ...
            VALUES (?, ?, ?, ?, ?)
        """, (sender['id'], recipient['id'], message, created_date.strftime('%Y-%m-%d %H:%M:%S'), is_read))
...
def create_notifications(users, count=40):
    ...
    notification_types = ['work_order_assigned', 'parts_arrived', 'training_due', 'quality_alert', 'system']
    ...
    for i in range(count):
        ...
        cur.execute("""
            ...
            VALUES (?, ?, ?, ?, ?)
        """, (user['id'], notif_type, messages[notif_type], created_date.strftime('%Y-%m-%d %H:%M:%S'), is_read))
...
def create_user_skills(users):
    ...
    skills = [
        "Hydraulic Systems", "Electrical Systems", "Mechanical Repair", "Welding",
        "PLC Programming", "Pneumatics", "HVAC", "Conveyor Systems", "Pump Maintenance",
        "Motor Repair", "Vibration Analysis", "Lubrication", "Alignment", "Balancing"
    ]
