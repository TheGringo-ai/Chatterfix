...
from .autogen_framework import AIAgent, AgentConfig, ModelType, AutogenOrchestrator
...
class DeploymentAgent(AIAgent):
    ...
    def __init__(self, config: AgentConfig, deployment_config: Optional[DeploymentConfig] = None):
...
class CloudOpsAgent(DeploymentAgent):
    ...
    def _classify_task(self, prompt: str) -> DeploymentTaskType:
        ...
        if any(keyword in prompt_lower for keyword in ["build", "deploy", "cloudbuild"]):
            return DeploymentTaskType.CLOUD_BUILD
        elif any(keyword in prompt_lower for keyword in ["infrastructure", "terraform", "gcp"]):
    ...
    async def _handle_cloud_build(self, prompt: str, context: str) -> str:
        try:
            ...
            gcloud_check = await self.execute_command("gcloud config list --format=json")
            ...
            if current_project != self.deployment_config.project_id:
                set_project = await self.execute_command(f"gcloud config set project {self.deployment_config.project_id}")
            ...
            services_check = await self.execute_command("gcloud services list --enabled --filter='name:cloudbuild.googleapis.com' --format='value(name)'")
            
            if "cloudbuild.googleapis.com" not in services_check["stdout"]:
                enable_result = await self.execute_command("gcloud services enable cloudbuild.googleapis.com")
...
class GitHubOpsAgent(DeploymentAgent):
    ...
    def _classify_github_task(self, prompt: str) -> DeploymentTaskType:
        prompt_lower = prompt.lower()
        if any(keyword in prompt_lower for keyword in ["deploy", "workflow", "actions"]):
    ...
    async def _handle_github_deployment(self, prompt: str, context: str) -> str:
        try:
            ...
            has_changes = bool(repo_check["stdout"].strip()) if repo_check["success"] else False
            
            # Get current branch
            branch_check = await self.execute_command("git branch --show-current")
            current_branch = branch_check["stdout"].strip() if branch_check["success"] else "unknown"
            
            # Check if GitHub Actions workflow exists
            workflow_exists = os.path.exists(".github/workflows/deploy.yml") or os.path.exists(".github/workflows/deploy-cloud-run.yml")
...
class DeploymentOrchestrator(AutogenOrchestrator):
    ...
    def setup_deployment_team(self):
        ...
        deployment_agents_config = [
            AgentConfig("cloudops-manager", ModelType.GROK, "Cloud Operations Manager", 
                       ["gcp", "cloud-build", "infrastructure"], "grok-3"),
            AgentConfig("github-specialist", ModelType.CHATGPT, "GitHub Operations Specialist", 
                       ["github", "ci-cd", "workflows"]),
            AgentConfig("security-auditor", ModelType.CLAUDE, "Security & Compliance Auditor", 
                       ["security", "compliance", "auditing"]),
            AgentConfig("deployment-strategist", ModelType.GEMINI, "Deployment Strategy Advisor", 
...
def get_deployment_orchestrator(deployment_config: Optional[DeploymentConfig] = None) -> DeploymentOrchestrator:
