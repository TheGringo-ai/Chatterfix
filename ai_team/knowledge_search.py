...
from .memory_system import ComprehensiveMemorySystem, get_memory_system
...
class AdvancedKnowledgeSearch:
    ...
    async def build_searchable_index(
        ...
        try:
            ...
            logger.info(
                f"✅ Knowledge index built successfully: {len(searchable_content)} items indexed"
            )
            return {
                "status": "success",
    ...
    async def search_knowledge(
        ...
        try:
            ...
            exact_results = await self._keyword_search(
                query, content_types, project_context, date_range
            )
    ...
    async def find_solution_patterns(
        self, problem_description: str, similarity_threshold: float = 0.3
    ) -> List[SearchResult]:
        """Find solution patterns for similar problems"""
        try:
            # Search specifically for solutions
            results = await self.search_knowledge(
                query=problem_description, content_types=["solution"], limit=10
            )

            # Filter by similarity threshold
            filtered_results = [
                r for r in results if r.relevance_score >= similarity_threshold
            ]
    ...
    async def find_mistake_patterns(
        self, action_description: str, risk_threshold: float = 0.4
    ) -> List[SearchResult]:
        """Find mistake patterns that might be relevant to current action"""
        try:
            # Search specifically for mistakes
            results = await self.search_knowledge(
                query=action_description, content_types=["mistake"], limit=10
            )
    ...
    async def _index_conversations(
        self, searchable_content: List[KnowledgeIndex]
    ) -> int:
            ...
            for doc in docs:
                ...
                index_item = KnowledgeIndex(
                    ...
                    metadata={
                        ...
                        "performance_metrics": solution_data.get(
                            "performance_metrics", {}
                        ),
                        "timestamp": datetime.now(
                            timezone.utc
                        ),  # Solutions don't have creation time
                    },
                    last_updated=datetime.now(timezone.utc),
                )
    ...
    async def _keyword_search(
        ...
        try:
            ...
            for doc in docs:
                ...
                if (
                    content_types
                    and index_data.get("content_type") not in content_types
                ):
                    continue
                ...
                if (
                    project_context
                    and index_data.get("metadata", {}).get("project_context")
                    != project_context
                ):
                    continue
                ...
                if relevance_score > 0.3:  # Minimum threshold
                    result = SearchResult(
                        ...
                        timestamp=index_data.get("metadata", {}).get(
        ...
        try:
            ...
            for i, similarity_score in enumerate(similarities):
                if similarity_score > 0.2:  # Minimum similarity threshold
                    ...
                    if (
                        project_context
                        and metadata["metadata"].get("project_context")
                        != project_context
                    ):
                        continue
                    ...
                    result = SearchResult(
                        ...
                        timestamp=metadata["metadata"].get(
                            "timestamp", datetime.now(timezone.utc)
                        ),
                    )
                    results.append(result)
    ...
    async def _index_is_current(self) -> bool:
        ...
        try:
            metadata_ref = self.db.collection("system_metadata").document(
                "knowledge_index"
            )
    ...
    async def _store_index_metadata(self, metadata: Dict[str, Any]):
        ...
        try:
            ...
            for i, doc_meta in enumerate(self.document_metadata):
                index_ref = self.db.collection(self.index_collection).document(
                    f"item_{i}"
                )
                index_ref.set(
                    {
