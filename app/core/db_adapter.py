...
class DatabaseAdapter:
    ...
    async def get_work_orders(self, status: str = None, assigned_to: str = None) -> List[Dict[str, Any]]:
        """Get work orders"""
        if self.use_firestore:
    ...
    async def get_assets(self, status: str = None, location: str = None) -> List[Dict[str, Any]]:
        """Get assets"""
        if self.use_firestore:
    ...
    async def save_ai_interaction(self, user_message: str, ai_response: str) -> str:
        """Save AI interaction"""
        if self.use_firestore:
            return await self.firestore_manager.save_ai_interaction(user_message, ai_response)
        else:
    ...
    async def get_dashboard_data(self, user_id: str = None) -> Dict[str, Any]:
        """Get dashboard data"""
        if self.use_firestore:
            return await self.firestore_manager.get_dashboard_data(user_id or "anonymous")
        else:
    ...
    def _get_sqlite_work_orders(self, status: str = None, assigned_to: str = None) -> List[Dict[str, Any]]:
    ...
    def _get_sqlite_assets(self, status: str = None, location: str = None) -> List[Dict[str, Any]]:
    ...
    def _get_sqlite_dashboard_data(self, user_id: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            work_orders_query = "SELECT * FROM work_orders ORDER BY created_date DESC LIMIT 10"
            if user_id:
            ...
            cursor.execute("SELECT * FROM ai_interactions ORDER BY timestamp DESC LIMIT 5")
