...
def role_required(allowed_roles: List[str]) -> Callable:
    ...
    async def check_role(current_user: UserInDB = Depends(get_current_user)) -> UserInDB:
        ...
        logger.info(f"Access granted to user {current_user.username} with role {current_user.role}")
        return current_user
...
def permission_required(required_permissions: List[str]) -> Callable:
    ...
    async def check_permissions(current_user: UserInDB = Depends(get_current_user)) -> UserInDB:
        ...
        logger.info(f"Permission granted to user {current_user.username} for {required_permissions}")
        return current_user
