...
class SensorManager:
    ...
    @require_iot_license
    async def add_sensor(self, customer_id: str, sensor_config: SensorConfig) -> Dict[str, Any]:
    ...
    async def _create_modbus_collector(self, config: SensorConfig):
                ...
                for register in self.registers:
                    try:
                        ...
                        if result.isError():
                            ...
                            scaled_value = raw_value * register.get("scale", 1.0) + register.get("offset", 0.0)
        ...
        if await collector.connect():
            ...
            raise ConnectionError(f"Failed to connect to Modbus device at {connection_params['host']}")
    ...
    async def _create_mqtt_collector(self, config: SensorConfig):
        ...
        if await collector.connect():
            ...
            raise ConnectionError(f"Failed to connect to MQTT broker at {connection_params['broker']}")
        ...
        while device.config.enabled:
            try:
                ...
                if device.connection_errors >= 3:
                    ...
                    logger.warning(f"Sensor {sensor_id} marked as ERROR after {device.connection_errors} failures")
                
                # Wait before retry (exponential backoff)
                retry_delay = min(300, 30 * (2 ** (device.connection_errors - 1)))
                await asyncio.sleep(retry_delay)
    
    async def _process_sensor_reading(self, sensor_id: str, raw_data: Any) -> Optional[SensorReading]:
    ...
    async def _store_sensor_reading(self, reading: SensorReading):
        ...
        logger.debug(f"Storing reading: {reading.sensor_id} = {reading.value} {reading.unit}")
    ...
    @require_iot_license  
    async def get_sensor_status(self, customer_id: str, sensor_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if sensor_id:
            if sensor_id in self.sensors:
                ...
                return {
                    ...
                    "status": device.status.value,
                    "last_reading": asdict(device.last_reading) if device.last_reading else None,
            ...
            for device in self.sensors.values():
                sensors_status.append({
                    ...
                    "status": device.status.value,
                    "last_reading": device.last_reading.value if device.last_reading else None,
                    "last_seen": device.last_seen.isoformat() if device.last_seen else None,
                    "alert_level": device.last_reading.alert_level if device.last_reading else "unknown"
                })
            
            return {
                "total_sensors": len(self.sensors),
                "online_sensors": len([s for s in self.sensors.values() if s.status == SensorStatus.ONLINE]),
                "sensors": sensors_status
            }
    
    @require_iot_license
    async def get_sensor_data(self, customer_id: str, sensor_id: str, hours: int = 24) -> Dict[str, Any]:
