...
async def chat(
    ...
    """General AI Chat"""
    response = await chatterfix_ai.process_message(message, context, user_id=current_user['id'])
...
async def analyze_image(
    ...
    try:
        response = await chatterfix_ai.gemini.analyze_image(temp_path, prompt, user_id=current_user['id'])
    finally:
...
async def kpi_report(current_user: dict = Depends(get_current_user)):
    ...
    report = await chatterfix_ai.gemini.generate_kpi_report(data, user_id=current_user['id'])
...
async def troubleshoot(
    ...
    advice = await chatterfix_ai.gemini.get_troubleshooting_advice(asset, issue, user_id=current_user['id'])
...
async def assist(
    ...
    try:
        ...
        result = await chatterfix_ai.gemini.run_assistant_agent(message, context, user_id=current_user['id'])
