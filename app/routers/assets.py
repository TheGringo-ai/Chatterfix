...
async def assets_list(request: Request):
    ...
    stats = {
        ...
        'maintenance_due': len([a for a in assets if a['next_service_date'] and a['next_service_date'] <= datetime.now().strftime('%Y-%m-%d')])
    }
...
async def asset_detail(request: Request, asset_id: int):
    ...
    # Get child assets
    children = conn.execute("SELECT * FROM assets WHERE parent_asset_id = ?", (asset_id,)).fetchall()
    
    # Get media
    media = conn.execute("SELECT * FROM asset_media WHERE asset_id = ? ORDER BY uploaded_date DESC", (asset_id,)).fetchall()
        ...
        for file in files:
            if file.filename:
                ...
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """, (asset_id, rel_path, file_type, file.filename, "Uploaded during creation"))
...
async def ai_health_analysis(asset_id: int):
    ...
    return JSONResponse({
        ...
        "recommendations": ["Schedule preventive maintenance", "Monitor performance metrics"]
    })
...
async def ai_recommendations(asset_id: int):
    ...
    try:
        ...
        return JSONResponse({
            "recommendations": [
                {"task": "Inspect bearings", "priority": "High", "estimated_cost": 150},
                {"task": "Replace filters", "priority": "Medium", "estimated_cost": 75}
            ]
        })
