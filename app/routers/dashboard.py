...
async def dashboard(request: Request, session_token: Optional[str] = Cookie(None), user_id: int = None, current_user: dict = None):
    ...
    try:
        ...
        performance = {"today": {"completion_rate": 0, "total_work_orders": 0, "completed_today": 0}}
        notifications = {"unread_count": 0, "recent_interactions": []}
        equipment = {"total_assets": 0, "healthy": 0, "warning": 0, "critical": 0, "uptime_percentage": 100}
...
@router.post("/config")
async def save_dashboard_config(layout: DashboardLayoutUpdate, session_token: Optional[str] = Cookie(None)):
...
async def get_available_widgets(session_token: Optional[str] = Cookie(None)):
    ...
    default_widgets = [
        {"id": 1, "widget_type": "workload", "title": "Workload Overview", "description": "Active work orders and assignments", "default_roles": "all"},
        {"id": 2, "widget_type": "performance", "title": "Performance Metrics", "description": "Completion rates and efficiency", "default_roles": "manager,technician"},
        {"id": 3, "widget_type": "notifications", "title": "Notifications", "description": "Recent alerts and messages", "default_roles": "all"},
        {"id": 4, "widget_type": "equipment", "title": "Equipment Status", "description": "Asset health and maintenance needs", "default_roles": "all"},
        {"id": 5, "widget_type": "analytics", "title": "Analytics", "description": "Reports and insights", "default_roles": "manager,admin"}
    ]
...
async def get_widget_data(widget_type: str, session_token: Optional[str] = Cookie(None)):
...
async def update_widget_config(widget_id: int, config: dict, session_token: Optional[str] = Cookie(None)):
