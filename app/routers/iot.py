...
async def add_sensor(
    ...
    try:
        ...
        return JSONResponse(
            {"success": False, "error": f"Failed to add sensor: {str(e)}"}
        )
...
async def get_dashboard_overview(current_user: User = Depends(require_auth_cookie)):
    ...
    async def _get_overview():
        ...
        return {
            ...
            "system_health": (
                "good"
                if online_sensors == total_sensors
                else "warning" if online_sensors > 0 else "critical"
            ),
        }
...
async def process_voice_sensor_query(
    ...
    async def _process_voice_query():
        ...
        if "temperature" in voice_lower:
            ...
            temp_sensors = [
                s
                for s in sensor_status.get("sensors", [])
            ...
            if temp_sensors:
                ...
                for sensor in temp_sensors[:3]:
                    if sensor.get("last_reading"):
                        responses.append(
                            f"{sensor['name']}: {sensor['last_reading']} degrees"
                        )

                voice_response = (
                    f"Temperature readings: {', '.join(responses)}"
                    if responses
                    else "No recent temperature readings available"
                )
            else:
            ...
            if pressure_sensors:
                ...
                for sensor in pressure_sensors[:3]:
                    if sensor.get("last_reading"):
                        responses.append(
                            f"{sensor['name']}: {sensor['last_reading']} PSI"
                        )

                voice_response = (
                    f"Pressure readings: {', '.join(responses)}"
                    if responses
                    else "No recent pressure readings available"
                )
            else:
            ...
            "templates": [
                {
                    ...
                    "data_mapping": {
                        "registers": [{"address": 40001, "scale": 0.1, "offset": 0.0}],
                        "unit": "°C",
                    },
                    "alert_thresholds": {"warning_high": 80.0, "critical_high": 95.0},
                },
                {
                    ...
                    "data_mapping": {"value_field": "pressure", "unit": "PSI"},
                    "alert_thresholds": {"warning_high": 150.0, "critical_high": 175.0},
                },
            ],
            "iot_available": IOT_AVAILABLE,
