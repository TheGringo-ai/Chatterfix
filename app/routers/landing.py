...
async def send_welcome_email(user_data: dict, password: str):
    ...
    try:
        # Simplified logging for now - email functionality can be added later
        logger.info(f"Welcome email logged for {user_data['fullName']} ({user_data['email']})")
...
async def handle_landing_signup(
    signup_data: LandingSignupRequest,
    ...
    try:
        ...
        logger.info(f"New user created: {signup_data.email} for company {signup_data.company}")
