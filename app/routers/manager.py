...
async def assets_table_view(request: Request):
    ...
    try:
        ...
        if db_adapter.firestore_manager:
            assets_data = await db_adapter.firestore_manager.get_collection("assets", order_by="name")
            assets = assets_data
        else:
            # Mock data
            assets = [
...
async def parts_table_view(request: Request):
    ...
    try:
        ...
        if db_adapter.firestore_manager:
            parts_data = await db_adapter.firestore_manager.get_collection("parts", order_by="name")
            parts = parts_data
        else:
            # Mock data
            parts = [
...
async def bulk_delete(
    ...
    try:
        ...
        for entity_id in ids_list:
            try:
                if db_adapter.firestore_manager:
                    await db_adapter.firestore_manager.delete_document(entity_type, entity_id)
                    success_count += 1
                else:
                    # Mock delete
                    success_count += 1
        ...
        for row_num, row in enumerate(csv_reader, start=2):
            try:
                ...
                if db_adapter.firestore_manager:
                    ...
                    results.append({"row": row_num, "id": wo_id, "title": work_order_data["title"]})
                    success_count += 1
                else:
                    # Mock creation
                    results.append({"row": row_num, "id": f"wo_{row_num}", "title": work_order_data["title"]})
                    success_count += 1
        ...
        for row_num, row in enumerate(csv_reader, start=2):
            try:
                ...
                if db_adapter.firestore_manager:
                    ...
                    results.append({"row": row_num, "id": part_id, "name": part_data["name"]})
                    success_count += 1
                else:
                    results.append({"row": row_num, "id": f"part_{row_num}", "name": part_data["name"]})
                    success_count += 1
        ...
        for row_num, row in enumerate(csv_reader, start=2):
            try:
                ...
                if db_adapter.firestore_manager:
                    ...
                    results.append({"row": row_num, "id": pm_id, "task": pm_data["task_name"]})
                    success_count += 1
                else:
                    results.append({"row": row_num, "id": f"pm_{row_num}", "task": pm_data["task_name"]})
                    success_count += 1
...
async def technician_analytics(request: Request):
    ...
    return templates.TemplateResponse(
        "manager_technician_analytics.html",
        {"request": request}
    )
...
async def get_technician_performance_data(
    ...
    try:
        ...
        # Apply filters
        if department != "all":
            performance_data = [t for t in performance_data if t["department"].lower() == department]
            
        if performance_filter != "all":
            if performance_filter == "excellent":
                performance_data = [t for t in performance_data if t["efficiency_rating"] >= 90]
            elif performance_filter == "good":
                performance_data = [t for t in performance_data if 80 <= t["efficiency_rating"] < 90]
            elif performance_filter == "average":
                performance_data = [t for t in performance_data if 70 <= t["efficiency_rating"] < 80]
            elif performance_filter == "needs-improvement":
                performance_data = [t for t in performance_data if t["efficiency_rating"] < 70]
        ...
        return JSONResponse({
            ...
            "summary": {
                ...
                "avg_efficiency": sum(t["efficiency_rating"] for t in performance_data) / len(performance_data) if performance_data else 0,
                "avg_response_time": sum(t["avg_response_time"] for t in performance_data) / len(performance_data) if performance_data else 0,
                "avg_rating": sum(t["customer_rating"] for t in performance_data) / len(performance_data) if performance_data else 0
            }
        })
...
async def get_pm_schedules(
    ...
    try:
        ...
        if asset_type != "all":
            pm_schedules = [s for s in pm_schedules if s["asset_type"].lower() == asset_type.lower()]
        ...
        return JSONResponse({
            ...
            "summary": {
                ...
                "active_schedules": len([s for s in pm_schedules if s["status"] == "active"]),
                "overdue_schedules": len([s for s in pm_schedules if s["status"] == "overdue"]),
                "automation_rate": len([s for s in pm_schedules if s["automation_enabled"]]) / len(pm_schedules) * 100 if pm_schedules else 0
            }
        })
...
async def run_pm_schedule_now(schedule_id: str):
    ...
    try:
        ...
        if db_adapter.firestore_manager:
            schedule = await db_adapter.firestore_manager.get_document("pm_schedules", schedule_id)
        else:
            # Mock schedule data
            schedule = {
                "asset_id": "hvac12",
