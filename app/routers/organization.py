...
async def update_organization(
    ...
    success = await org_service.update_organization(current_user.organization_id, update_dict)
    if not success:
...
async def invite_team_member(
    ...
    try:
        ...
        org = await org_service.get_organization(current_user.organization_id)
        org_name = org.get("name", "ChatterFix Organization") if org else "ChatterFix Organization"
        ...
        logger.info(f"Invited {invite_data.email} to org {current_user.organization_id} as {invite_data.role}")
...
async def get_pending_invites(current_user: User = Depends(require_permission("manage_team"))):
...
async def view_invite(request: Request, invite_code: str):
    ...
    # Get invite details
    if org_service.db:
        invite_ref = org_service.db.collection("organization_invites").document(invite_code)
            ...
            if invite.get("status") == "pending":
                return templates.TemplateResponse(
                    "organization/accept_invite.html",
                    {
                        ...
                        "invite": invite,
                        "invite_code": invite_code,
                        "organization_name": invite.get("organization_name", "An organization"),
                    }
                )

    # Invalid or expired invite
    return templates.TemplateResponse(
        "organization/invalid_invite.html",
        {"request": request}
    )
...
async def api_get_organization_info(current_user: User = Depends(get_current_active_user)):
