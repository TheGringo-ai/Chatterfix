    ...
    try:
        ...
        for asset_id, asset in mock_data["assets"].items():
            serialized_data["assets"][asset_id] = {
                ...
                ] if asset.maintenance_window else None
            }
...
async def optimize_schedule(
    ...
    objectives: Optional[List[str]] = Query(None, description="Optimization objectives")
):
...
async def get_technician_schedule(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get technician schedule: {str(e)}")
    ...
    try:
        ...
        for skill, levels in skill_coverage.items():
            skill_stats[skill] = {
                ...
                "certified_count": sum(1 for tech in technicians.values() 
        ...
        return JSONResponse(content={
            "technician_analytics": {
                ...
                "average_hourly_rate": round(sum(t.hourly_rate for t in technicians.values()) / len(technicians), 2)
            },
            "asset_analytics": {
            ...
            "locations": list(set(tech_by_location.keys()) | set(asset_by_location.keys()))
        })
...
async def get_pm_automation_overview(days_ahead: int = Query(30, description="Days to look ahead")):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get PM overview: {str(e)}")
...
async def generate_pm_schedule(
    ...
    try:
        ...
        generated_orders = await pm_automation_engine.generate_pm_schedule(start_dt, end_dt)
        ...
        raise HTTPException(status_code=500, detail=f"Failed to generate PM schedule: {str(e)}")
...
async def update_meter_reading(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to update meter reading: {str(e)}")
...
async def get_maintenance_templates():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get maintenance templates: {str(e)}")


@router.get("/pm-automation/asset-meters")
async def get_asset_meters(asset_id: Optional[str] = Query(None, description="Filter by asset ID")):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get asset meters: {str(e)}")
