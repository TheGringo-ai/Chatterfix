...
from app.models.permissions import Role, Permission, PERMISSION_MATRIX
from app.utils.auth import (
    hash_password, verify_password, create_access_token, verify_token,
    validate_password_strength, create_audit_log_entry, check_rate_limit,
    session_manager, SecurityError
)
...
async def login_for_access_token(
    ...
    try:
        ...
        rate_allowed, rate_info = check_rate_limit(
            user_id=hash(form_data.username) % 10000,  # Simple user ID for rate limiting
            endpoint="auth/token",
        ...
        if not rate_allowed:
            logger.warning(f"Rate limit exceeded for login attempt: {form_data.username}")
            raise HTTPException(
        ...
        session_id = session_manager.create_session(
            user_id=user.id,
            user_agent=form_data.client_id or "",  # OAuth2 doesn't have user_agent, using client_id
