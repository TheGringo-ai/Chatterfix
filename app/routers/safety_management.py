...
class SafetyIncident(BaseModel):
    ...
    incident_type: str = Field(
        ..., description="Near miss, injury, property damage, etc."
    )
...
async def safety_dashboard(request: Request):
    ...
    open_violations = len(
        [v for v in safety_violations if v["status"] in ["Open", "In Progress"]]
    )
    non_compliant_tests = len(
        [r for r in lab_results if r["compliance_status"] == "Non-compliant"]
    )
...
async def get_safety_violations():
    ...
    return JSONResponse(
        {"violations": safety_violations, "total": len(safety_violations)}
    )
...
async def get_safety_inspections():
    ...
    return JSONResponse(
        {"inspections": safety_inspections, "total": len(safety_inspections)}
    )
        ...
        ai_analysis = {
                ...
                "high_risk_areas": [
                    {
                        ...
                        "risk_score": 8.7,
                        "primary_hazards": ["Fire", "Fumes", "Burns"],
                    },
                    {
                        "location": "Chemical Storage",
                        "risk_score": 8.2,
                        "primary_hazards": ["Chemical Exposure", "Spills"],
                    },
                    {
                        "location": "Machine Shop",
                        "risk_score": 7.9,
                        "primary_hazards": ["Noise", "Cuts", "Eye Injury"],
                    },
                ],
                "trending_risks": [
            ...
            "predictive_insights": {
                ...
                "equipment_safety_concerns": [
                    {
                        "equipment": "Press #3",
                        "concern": "Guard issues",
                        "urgency": "High",
                    },
                    {
                        "equipment": "Conveyor B",
                        "concern": "Pinch points",
                        "urgency": "Medium",
                    },
                ],
            },
...
def analyze_lab_result_with_ai(result_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        analysis = {
            ...
            "risk_level": (
                "Low"
                if percentage_of_limit < 70
                else "Medium" if percentage_of_limit < 90 else "High"
            ),
            "recommendations": [],
        }
