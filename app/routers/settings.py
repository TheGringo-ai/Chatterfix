...
async def users_management(request: Request, session_token: Optional[str] = Cookie(None)):
...
async def add_user(
    ...
    if user_id:
        return JSONResponse({"success": True, "message": "User created successfully", "user_id": user_id})
    else:
        return JSONResponse({"success": False, "message": "Username or email already exists"}, status_code=400)
