...
async def save_api_keys(
    ...
    try:
        ...
        existing_settings = await firestore.get_document("user_settings", current_user.uid) or {}
        ...
        return JSONResponse({"success": False, "message": f"Failed to save: {str(e)}"}, status_code=500)
...
async def users_management(
    ...
    try:
        ...
        users = [
            {"id": "demo-1", "username": "demo_user", "email": "user@demo.com", "full_name": "Demo User", "role": "technician", "status": "active", "is_active": True, "created_date": "2024-01-01"},
        ]
...
async def toggle_user_active(user_id: str, current_user: User = Depends(require_permission("manager"))):
    ...
    try:
        ...
        # Update in Firestore
        await firestore.update_document("users", user_id, {
            "is_active": new_status,
            "updated_at": __import__('datetime').datetime.now(__import__('datetime').timezone.utc),
            "updated_by": current_user.uid,
        })

        logger.info(f"User {user_id} status toggled to {new_status} by manager {current_user.uid}")
        ...
        return JSONResponse({"success": False, "message": f"Failed to update: {str(e)}"}, status_code=500)
