...
async def websocket_endpoint(websocket: WebSocket, user_id: int):
    ...
    conn = get_db_connection()
    conn.execute("UPDATE users SET status = 'available', last_seen = ? WHERE id = ?",
    ...
    try:
        while True:
            ...
            if data['type'] == 'chat_message':
                ...
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, 'chat', ?, ?)
                """, (user_id, data.get('recipient_id'), data.get('work_order_id'),
                ...
                if data.get('recipient_id'):
                    await websocket_manager.send_personal_message(message_data, data['recipient_id'])
                else:
                    await websocket_manager.broadcast(message_data, exclude_user=user_id)
        ...
        conn = get_db_connection()
        conn.execute("UPDATE users SET status = 'offline', last_seen = ? WHERE id = ?",
                    (datetime.now(), user_id))
        conn.commit()
        conn.close()
