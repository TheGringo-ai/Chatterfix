...
async def module_detail(request: Request, module_id: int):
    ...
    try:
        ...
        return templates.TemplateResponse("training_module.html", {
            "request": request,
            "module": module,
            "content": content
        })
...
async def generate_quick_guide(
    ...
    guide = await training_generator.generate_quick_guide(equipment_name, task_description)
    return {"guide": guide}
...
async def assign_training(
    ...
    # Get module title
    conn = get_db_connection()
    module = conn.execute("SELECT title FROM training_modules WHERE id = ?", (module_id,)).fetchone()
    conn.close()
