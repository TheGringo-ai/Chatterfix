...
async def work_order_detail(request: Request, wo_id: int):
    ...
    media = conn.execute("SELECT * FROM work_order_media WHERE work_order_id = ? ORDER BY uploaded_date DESC", (wo_id,)).fetchall()
    conn.close()
    
    return templates.TemplateResponse("work_order_detail.html", {"request": request, "wo": wo, "media": media})
