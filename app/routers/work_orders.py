...
@router.get("/linesmart", response_class=HTMLResponse)
async def linesmart_dashboard(request: Request):
    """Render LineSmart Intelligence Dashboard"""
    try:
        return templates.TemplateResponse(
            "linesmart_dashboard.html", {"request": request}
        )
    except Exception as e:
        logger.error(f"Failed to load LineSmart dashboard: {e}")
        return templates.TemplateResponse(
            "linesmart_dashboard.html",
            {"request": request, "error": "Failed to load LineSmart dashboard"},
        )
...
async def complete_work_order_enhanced(
    ...
    try:
        ...
        # **LINESMART INTELLIGENCE**: Analyze completed work order for training opportunities
        try:
            logger.info(f"Starting LineSmart intelligence analysis for work order {wo_id}")
            intelligence_analysis = await linesmart_intelligence.analyze_failure_for_training(wo_id)
            
            if intelligence_analysis.get("training_opportunities"):
                logger.info(f"LineSmart identified training opportunities for work order {wo_id}")
                
                # If training recommendation is available, auto-assign it to technician
                training_recommendation = intelligence_analysis.get("training_recommendation")
                if training_recommendation and wo.get("assigned_to"):
                    try:
                        module_id = await linesmart_intelligence.assign_intelligent_training(
                            wo["assigned_to"], training_recommendation
                        )
                        logger.info(f"Auto-assigned LineSmart training module {module_id} to technician {wo['assigned_to']}")
                    except Exception as e:
                        logger.error(f"Failed to auto-assign LineSmart training: {e}")
            else:
                logger.info(f"No training opportunities identified for work order {wo_id}")
                
        except Exception as e:
            logger.error(f"LineSmart intelligence analysis failed for work order {wo_id}: {e}")
            # Don't fail the completion process if intelligence analysis fails
...
# LineSmart Intelligence API Endpoints

@router.get("/api/linesmart/analytics")
async def get_linesmart_analytics(timeframe_days: int = 90):
    """Get LineSmart workforce intelligence analytics"""
    try:
        analytics = await linesmart_intelligence.get_skill_gap_analytics(timeframe_days=timeframe_days)
        return JSONResponse(analytics)
    except Exception as e:
        logger.error(f"Error getting LineSmart analytics: {e}")
        return JSONResponse({"error": str(e)}, status_code=500)


@router.get("/api/linesmart/dashboard")
async def get_linesmart_dashboard(timeframe_days: int = 90):
    """Get comprehensive LineSmart workforce intelligence dashboard"""
    try:
        ...
        logger.error(f"Error getting LineSmart dashboard: {e}")
        return JSONResponse({"error": str(e)}, status_code=500)


@router.get("/api/linesmart/technician/{technician_id}/performance")
async def get_technician_performance(technician_id: str, timeframe_days: int = 180):
    """Get performance improvement metrics for a specific technician"""
    try:
        metrics = await linesmart_intelligence.get_performance_improvement_metrics(
            technician_id, timeframe_days
        )
...
@router.post("/api/linesmart/analyze/{wo_id}")
async def manual_intelligence_analysis(wo_id: str):
    """Manually trigger LineSmart intelligence analysis for a work order"""
    try:
