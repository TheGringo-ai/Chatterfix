...
class AIOrchestrator:
    ...
    def _is_model_available(self, model: AIModel) -> bool:
        ...
        try:
            if model == AIModel.GEMINI:
                return hasattr(self.gemini_service, 'default_api_key') and self.gemini_service.default_api_key
        ...
        for fallback_model in fallback_models:
            if self._is_model_available(fallback_model):
                try:
                    ...
                    logger.warning(f"⚠️ Fallback model {fallback_model.value} also failed: {e}")
    ...
    def _update_metrics(self, model: str, execution_time: float, success: bool):
        ...
        if model not in self.performance_metrics:
            self.performance_metrics[model] = {
                "calls": 0, "avg_time": 0, "success_rate": 100
            }
    ...
    async def multi_ai_consensus(
        ...
        results = await asyncio.gather(
            *[task_coroutine for model, task_coroutine in tasks],
        ...
        return {
            ...
            "successful_responses": len([r for r in results if not isinstance(r, Exception)]),
            "task": task,
            "context": context
        }
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_requests": sum(m["calls"] for m in self.performance_metrics.values())
        }
