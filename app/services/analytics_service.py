...
class AnalyticsService:
    ...
    async def get_kpi_summary(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            has_comprehensive_data = (
                work_order_metrics.get("total_created", 0) >= 50
                and work_order_metrics.get("completion_rate", 0) > 50
                and cost_tracking.get("total_cost", 0) >= 1000
                and mttr.get("total_repairs", 0) >= 10
                and asset_utilization.get("total_assets", 0) >= 10
            )

            if not has_comprehensive_data:
                logger.info(
                    "Insufficient real data for complete dashboard - using demo data for display"
                )
    ...
    async def calculate_asset_utilization(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            for asset_id in active_asset_ids:
                asset_name = next(
                    (a["name"] for a in assets if a["id"] == asset_id), "Unknown"
                )
                downtime = sum(
                    h.get("downtime_hours", 0)
                    for h in history
            ...
            avg_utilization = (
                (total_uptime / (active_assets * total_hours)) * 100
                if active_assets > 0 and total_hours > 0
                else 100
            )
    ...
    async def get_cost_tracking(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_cost_per_event = (
                total_cost / maintenance_count if maintenance_count > 0 else 0
            )

            costs_by_type = {}
            for h in history:
                m_type = h.get("maintenance_type", "Unknown")
                costs_by_type[m_type] = costs_by_type.get(m_type, 0) + h.get(
                    "total_cost", 0
                )
            ...
            for h in history:
                ...
                if asset_id:
                    if asset_id not in costly_assets_agg:
                        costly_assets_agg[asset_id] = {
                            "total_cost": 0,
                            "events": 0,
                            "name": h.get("asset_name", asset_id),
                        }
                    costly_assets_agg[asset_id]["total_cost"] += h.get("total_cost", 0)
                    costly_assets_agg[asset_id]["events"] += 1

            top_costly_assets = sorted(
                costly_assets_agg.items(),
            ...
            for h in history:
                ...
                if date_str:
                    daily_trend_agg[date_str] = daily_trend_agg.get(
                        date_str, 0
                    ) + h.get("total_cost", 0)

            daily_trend = [
                {"date": d, "cost": round(c, 2)}
                for d, c in sorted(daily_trend_agg.items())
            ]
        ...
        try:
            ...
            for wo in work_orders:
                ...
                if due_date_str and status not in ["Completed", "Cancelled"]:
                    ...
                    if due_date.replace(tzinfo=timezone.utc) < datetime.now(
                        timezone.utc
                    ):
                        overdue_count += 1
            ...
            completion_rate = (
                (completed / total_created * 100) if total_created > 0 else 0
            )
    ...
    async def get_compliance_metrics(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            completed_wos, user_training, user_skills = await asyncio.gather(
                ...
                firestore_manager.get_collection(
                    "user_skills"
                ),  # Assuming a user_skills collection exists
            )
            ...
            total_pm = len(
                [
                    wo
                    for wo in completed_wos
                    if wo.get("work_order_type") == "Preventive"
                ]
            )
            on_time_pm = 0
            for wo in completed_wos:
                if (
                    wo.get("work_order_type") == "Preventive"
                    and wo.get("due_date")
                    and wo.get("completed_date")
                ):
            ...
            completed_training = len(
                [t for t in user_training if t.get("status") == "completed"]
            )
            training_compliance = (
            ...
            for skill in user_skills:
                if skill.get("certified") and skill.get("certification_expiry"):
                    ...
                    if expiry_date.replace(tzinfo=timezone.utc) >= datetime.now(
                        timezone.utc
                    ):
                        valid_certs += 1
                    else:
            ...
            return {
                ...
                "total_pm_work_orders": 0,
                "on_time_completions": 0,
                "training_compliance_rate": 0,
                "total_training_assigned": 0,
    ...
    async def _get_mttr_trend(
        self, firestore_manager, days: int
    ) -> List[Dict[str, Any]]:
        ...
        return [
            {
                "date": wo.get("completed_date", "").split("T")[0],
                "value": wo.get("actual_hours", 0),
            }
            for wo in completed_wos
        ]

    async def _get_mtbf_trend(
        self, firestore_manager, days: int
    ) -> List[Dict[str, Any]]:
    ...
    async def _get_cost_trend(
        ...
        for h in history:
            date_str = h.get("created_date", "").split("T")[0]
            if date_str:
                daily_costs[date_str] = daily_costs.get(date_str, 0) + h.get(
                    "total_cost", 0
                )
        return [
            {"date": d, "value": round(c, 2)} for d, c in sorted(daily_costs.items())
        ]

    async def _get_work_order_trend(
        self, firestore_manager, days: int
    ) -> List[Dict[str, Any]]:
    ...
    async def _calculate_trend(
        self, metric: str, current_value: float, days: int
    ) -> str:
        """
    ...
    def _get_default_mttr(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "trend": "improving",
            "status": "good",
            "message": "Demo: MTTR improved 8.5% from last period",
        }
