...
async def extract_text_from_image(image_data: bytes) -> Dict[str, Any]:
    ...
    try:
        ...
        if easyocr_reader:
            try:
                ...
                for bbox, text, confidence in results:
                    if (
                        confidence > 0.3 and len(text.strip()) > 1
        ...
        if (
            _tesseract_available and len(extracted_texts) < 3
        ):  # If EasyOCR didn't find much
            try:
                ...
                for i, text in enumerate(ocr_data["text"]):
                    ...
                    if confidence > 30 and len(text.strip()) > 1:
                        ...
                        duplicate = any(
                            abs(et["bbox"][0][0] - x) < 10
                            and et["text"].lower() == text.lower()
                ...
                logger.info(
                    f"Tesseract added {len([t for t in extracted_texts if t['engine'] == 'tesseract'])} additional texts"
                )
...
async def detect_equipment_issues(image_data: bytes) -> Dict[str, Any]:
    ...
    try:
        ...
        recommendations = await _generate_maintenance_recommendations(
            detected_issues, overall_score
        )
        ...
        if VOICE_MEMORY_AVAILABLE:
            try:
                ...
                image_hash = hashlib.md5(
                    image_data[:1000], usedforsecurity=False
                ).hexdigest()[:16]
...
async def _detect_corrosion(image: np.ndarray) -> float:
    ...
    try:
        ...
        corrosion_score = min(
            1.0, (rust_percentage * 5.0) + (texture_variance / 10000.0)
        )

        logger.info(
            f"Corrosion analysis: {rust_percentage*100:.2f}% rust color, texture var: {texture_variance:.1f}"
        )
    ...
    try:
        ...
        if wear_detected:
            severity = (
                "moderate"
                if edge_density > 0.1
                else "minor" if edge_density > 0.05 else "low"
            )
...
async def _detect_leaks(image: np.ndarray) -> Dict[str, Any]:
    ...
    try:
        ...
        contours, _ = cv2.findContours(
            dark_areas, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
        ...
        if potential_leaks:
            ...
            severity = (
                "high"
                if leak_percentage > 0.05
                else "moderate" if leak_percentage > 0.02 else "low"
            )
...
async def _assess_structural_integrity(image: np.ndarray) -> Dict[str, Any]:
    ...
    try:
        ...
        crack_contours, _ = cv2.findContours(
            crack_thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
            ...
            if area > 100:  # Significant crack size
                ...
                if length > 20:  # Long crack
                    ...
                    severity = (
                        "high"
                        if length > 100
                        else "moderate" if length > 50 else "minor"
                    )
...
async def _generate_maintenance_recommendations(
    issues: List[Dict], overall_score: float
) -> List[str]:
    ...
    if "corrosion" in issue_types:
        ...
        recommendations.append(
            "🌡️ Check environmental conditions (humidity, temperature)"
        )

    if "wear" in issue_types:
        recommendations.append("🔧 Inspect and replace worn components")
        recommendations.append(
            "⚙️ Review operating procedures for excessive wear causes"
        )
    ...
    if "crack" in issue_types:
        recommendations.append(
            "🚨 CRITICAL: Stop equipment operation if structural cracks found"
        )
...
def _determine_urgency(issues: List[Dict], overall_score: float) -> str:
    ...
    if (
        any(issue.get("severity") == "critical" for issue in issues)
        or overall_score < 3.0
    ):
        return "immediate"
    elif (
        any(issue.get("severity") in ["high", "severe"] for issue in issues)
        or overall_score < 5.0
    ):
        return "urgent"
    elif (
        any(issue.get("severity") == "moderate" for issue in issues)
        or overall_score < 7.0
    ):
        return "normal"
    else:
        return "routine"
...
async def read_gauge_meter(
    image_data: bytes, gauge_type: str = "auto", voice_feedback: bool = True
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if lines is not None:
            for line in lines:
                ...
                line_center_dist = (
                    abs((y2 - y1) * center_x - (x2 - x1) * center_y + x2 * y1 - y2 * x1)
                    / ((y2 - y1) ** 2 + (x2 - x1) ** 2) ** 0.5
                )
    ...
    try:
        ...
        if unit in pressure_thresholds:
            if value > pressure_thresholds[unit]:
                ...
                alerts.append(
                    f"⚠️ NEGATIVE PRESSURE: {value} {unit} indicates possible leak"
                )

        elif unit in temp_thresholds:
            if value > temp_thresholds[unit]:
                alerts.append(
                    f"🌡️ HIGH TEMPERATURE: {value} {unit} may indicate overheating"
                )
        ...
        if confidence < 0.5:
            alerts.append(
                "📸 LOW CONFIDENCE: Consider retaking photo for clearer reading"
            )
