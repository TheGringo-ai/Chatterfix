...
class GeminiService:
    ...
    async def recognize_asset_from_image(
        self, image_path: str, context: str = "", user_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            try:
                ...
                if json_start >= 0 and json_end > json_start:
                    ...
                    logger.info(
                        f"✅ Asset recognized: {asset_data.get('asset_type', 'Unknown')} - {asset_data.get('manufacturer', 'Unknown brand')}"
                    )
    ...
    def _validate_asset_data(self, asset_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(asset_data["confidence_score"], (int, float)):
            ...
            asset_data["confidence_score"] = max(
                0.0, min(1.0, asset_data["confidence_score"])
            )
    ...
    def _extract_asset_info_from_text(
        self, text_response: str, image_path: str
    ) -> Dict[str, Any]:
        ...
        if any(word in text_lower for word in ["excellent", "new", "pristine"]):
            ...
            word in text_lower for word in ["poor", "damaged", "corroded", "rust"]
        ):
    ...
    async def create_asset_registration_workflow(
        self, image_path: str, location: str, user_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            registration_data = {
                ...
                "status": (
                    "Operational"
                    if asset_data["condition_assessment"] in ["Excellent", "Good"]
                    else "Needs Inspection"
                ),
            ...
            asset_identifier = f"{location}-{asset_data['asset_type']}"
            asset_tag = f"AST-{hash(asset_identifier) % 10000:04d}"
