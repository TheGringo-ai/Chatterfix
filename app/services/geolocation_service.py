...
class GeolocationService:
    ...
    async def check_location_permission(self, user_id: str) -> bool:
        ...
        settings = await self.firestore_manager.get_document(
    ...
    async def update_user_location(
        ...
        await self.firestore_manager.create_document(
            "user_location_history", history_data
        )
    ...
    async def is_within_property(self, latitude: float, longitude: float) -> bool:
        ...
        for boundary in boundaries:
            ...
            if boundary.get("boundary_type") == "polygon" and self._point_in_polygon(
                latitude, longitude, coords
            ):
                return True
            elif boundary.get("boundary_type") == "circle" and self._point_in_circle(
                latitude, longitude, coords
            ):
                return True
        return False
    ...
    async def get_nearby_work_orders(
        self, user_id: str, radius_meters: int = 500
    ) -> List[Dict]:
    ...
    async def add_property_boundary(
        ...
        return await self.firestore_manager.create_document(
            "property_boundaries", boundary_data
        )
