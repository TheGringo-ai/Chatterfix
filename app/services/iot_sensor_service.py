...
class IoTSensorService:
    ...
    async def record_sensor_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            reading_id = await self.firestore_manager.create_document(
                "sensor_readings", reading_data
            )
            ...
            if alert:
                ...
                if alert.get("severity") == "critical":
                    work_order = await self._create_automated_work_order(
                        reading_data, alert
                    )
    ...
    async def get_sensor_readings(
        self, asset_id: str = None, sensor_type: str = None, hours: int = 24
    ) -> List[Dict[str, Any]]:
        """Get sensor readings with optional filters"""
        filters = []
        if asset_id:
            filters.append({"field": "asset_id", "operator": "==", "value": asset_id})
        if sensor_type:
            filters.append(
                {"field": "sensor_type", "operator": "==", "value": sensor_type}
            )

        start_date = datetime.now(timezone.utc) - timedelta(hours=hours)
        filters.append(
            {"field": "recorded_at", "operator": ">=", "value": start_date.isoformat()}
        )
    ...
    async def get_asset_sensor_summary(self, asset_id: str) -> Dict[str, Any]:
        ...
        for reading in readings:
            ...
            if not sensor_type or sensor_type in sensors:
                continue  # Process latest reading only
            ...
            sensors[sensor_type] = {
                ...
                "last_updated": reading.get("recorded_at"),
    ...
    async def record_batch_sensor_data(self, readings: List[Dict[str, Any]]):
        return {"success": False, "error": "Not implemented"}

    async def update_thresholds(self, sensor_type: str, thresholds: Dict[str, float]):
        return {"success": False, "error": "Not implemented"}

    async def get_predictive_insights(self, asset_id: str):
    ...
    async def _create_automated_work_order(
        self, data: Dict[str, Any], alert: Dict[str, Any]
