...
class IoTSensorService:
    ...
    def record_batch_sensor_data(self, readings: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def get_sensor_readings(self, asset_id: int = None, sensor_type: str = None, 
    ...
    def get_asset_sensor_summary(self, asset_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            for reading in latest:
                ...
                avg_data = next((a for a in averages if a['sensor_type'] == sensor_type), None)
                ...
                sensors[sensor_type] = {
                    ...
                    "status": status,
                    "average_24h": round(avg_data['avg_value'], 2) if avg_data else None,
    ...
    def update_thresholds(self, sensor_type: str, thresholds: Dict[str, float]) -> Dict[str, Any]:
    ...
    def _analyze_sensor_trend(self, sensor_type: str, readings: List[Dict]) -> Optional[Dict[str, Any]]:
    ...
    def _generate_recommendations(self, insights: List[Dict], risk_score: float) -> List[str]:
        """Generate maintenance recommendations based on sensor insights"""
        recommendations = []
        
        if risk_score >= 70:
            recommendations.append("🚨 URGENT: Schedule immediate inspection - high risk of failure detected")
        elif risk_score >= 50:
        ...
        for insight in insights:
            ...
            if status == "critical":
                recommendations.append(f"🔴 {sensor_type.title()}: Critical level - immediate action required")
            elif status == "warning":
                recommendations.append(f"🟡 {sensor_type.title()}: Warning level - monitor closely")
            elif trend == "increasing" and days_to_warning is not None and days_to_warning <= 7:
                recommendations.append(f"📈 {sensor_type.title()}: Rising trend - expected to reach warning in ~{days_to_warning} days")
    ...
    def _create_alert(self, data: Dict[str, Any], severity: str, thresholds: Dict) -> Dict[str, Any]:
...
This work order was automatically generated by the IoT monitoring system.""",
            ...
            logger.info(f"Automated work order created: #{work_order['id']} - {work_order['title']}")
