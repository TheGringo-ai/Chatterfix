...
class LineSmartIntelligence:
    ...
    def _initialize_openai(self):
        ...
        if api_key:
            ...
            logger.warning("OPENAI_API_KEY not found - LineSmart will use fallback methods")
    ...
    async def analyze_failure_for_training(self, work_order_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if wo_data.get("asset_id"):
                asset_data = await firestore_manager.get_document("assets", wo_data["asset_id"])
            
            # Get technician performance history
            technician_id = wo_data.get("assigned_to")
            technician_history = await self._get_technician_history(technician_id) if technician_id else []
            
            # Analyze failure patterns and identify training opportunities
            analysis = await self._analyze_failure_patterns(wo_data, asset_data, technician_history)
            
            # Generate training recommendations if opportunities found
            if analysis.get("training_opportunities"):
                training_recommendation = await self._generate_training_content(analysis)
        ...
        try:
            ...
            history = await firestore_manager.get_collection(
                ...
                filters=[
                    ...
                    {"field": "completed_date", "operator": ">=", "value": six_months_ago}
                ],
    ...
    async def _analyze_failure_patterns(self, wo_data: Dict, asset_data: Dict, history: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis_context = {
                ...
                "asset": {
                    "name": asset_data.get("name", "Unknown") if asset_data else "Unknown",
                    "type": asset_data.get("type", "Unknown") if asset_data else "Unknown",
                    "criticality": asset_data.get("criticality", "Medium") if asset_data else "Medium"
                },
                "technician_history": [
                    {
                        "title": h.get("title", ""),
    ...
    async def _fallback_pattern_analysis(self, wo_data: Dict, asset_data: Dict, history: List[Dict]) -> Dict[str, Any]:
        ...
        description = (wo_data.get("description", "") + " " + wo_data.get("work_summary", "")).lower()
        ...
        if len(history) > 0:
            ...
            if any(keyword in desc for desc in recent_issues for keyword in failure_keywords.get(identified_category, [])):
        ...
        return {
            "failure_analysis": {
                "failure_type": "equipment failure" if identified_category != "other" else "other",
            ...
            } if has_training_opportunity else None,
            "business_impact": {
    ...
    async def _generate_training_content(self, analysis: Dict) -> Dict[str, Any]:
        ...
        primary_focus = training_opportunities.get("primary_focus", "General Maintenance")
        failure_analysis = analysis.get("failure_analysis", {})
        
        try:
            if self.openai_client:
                # Use OpenAI to generate targeted training content
                training_content = await self._generate_ai_training_content(primary_focus, failure_analysis)
            else:
                # Use existing Gemini training generator as fallback
                training_content = await self._generate_fallback_training_content(primary_focus, failure_analysis)
    ...
    async def _generate_ai_training_content(self, focus: str, failure_analysis: Dict) -> Dict[str, Any]:
    ...
    async def _generate_fallback_training_content(self, focus: str, failure_analysis: Dict) -> Dict[str, Any]:
        ...
        training_templates = {
            "electrical": {
                ...
                "checklist": ["1. Turn off power and verify with meter", "2. Check connections for corrosion/looseness", "3. Test insulation and grounding"],
                "inspection": ["• Scorch marks or discoloration", "• Loose or corroded connections", "• Damaged insulation or exposed wires"],
            ...
            "mechanical": {
                ...
                "checklist": ["1. Check for unusual noise or vibration", "2. Inspect alignment and fasteners", "3. Verify lubrication levels and cleanliness"],
                "inspection": ["• Excessive vibration or noise", "• Misalignment or loose fasteners", "• Inadequate or contaminated lubrication"],
            ...
            "hydraulic": {
                ...
                "checklist": ["1. Check fluid levels and pressure readings", "2. Inspect hoses and fittings for leaks", "3. Verify filter condition and cleanliness"],
                "inspection": ["• Fluid leaks or low levels", "• Contaminated or discolored fluid", "• Excessive pressure or temperature"],
        ...
        return {
            ...
            "difficulty_level": 2,
            "sections": [
                {"title": "3-Step Action Checklist", "content": "\n".join(template["checklist"]), "type": "checklist"},
                {"title": "Visual Inspection Points", "content": "\n".join(template["inspection"]), "type": "inspection_guide"},
                {"title": "Safety Reminders", "content": template["safety"], "type": "safety"},
                {"title": "When to Escalate", "content": template["escalation"], "type": "escalation"}
            ],
            "quiz": [
                {
                    "question": f"What is the first step in {category} system troubleshooting?",
                    "options": ["Start equipment and listen", "Check safety procedures", "Replace components", "Call supervisor"],
                    "correct_answer": 1,
                    "explanation": "Safety procedures must always be followed first"
                }
            ],
            "learning_objectives": [f"Prevent {category} failures", "Follow safety procedures", "Know when to escalate"]
        }
    ...
    async def _store_intelligence_analysis(self, work_order_id: str, analysis: Dict):
        ...
        try:
            ...
            await firestore_manager.create_document("linesmart_intelligence", intelligence_record)
    ...
    async def get_skill_gap_analytics(self, technician_id: Optional[str] = None, 
        ...
        try:
            ...
            filters = [{"field": "analysis_date", "operator": ">=", "value": cutoff_date}]
            if technician_id:
                ...
                technician_wos = await firestore_manager.get_collection("work_orders", filters=wo_filters)
                ...
                analyses = await firestore_manager.get_collection("linesmart_intelligence", filters=filters)
            ...
            for analysis in analyses:
                ...
                # Count failure types
                failure_type = analysis.get("failure_analysis", {}).get("root_cause_category", "unknown")
                ...
                if training and training.get("primary_focus"):
                    ...
                    training_opportunities[focus] = training_opportunities.get(focus, 0) + 1
            
            return {
                "timeframe_days": timeframe_days,
                "total_analyses": len(analyses),
                "top_skill_gaps": dict(sorted(skill_gaps.items(), key=lambda x: x[1], reverse=True)[:5]),
                "failure_type_distribution": failure_types,
                "training_focus_areas": dict(sorted(training_opportunities.items(), key=lambda x: x[1], reverse=True)),
    ...
    async def assign_intelligent_training(self, technician_id: str, 
                                        training_content: Dict[str, Any]) -> str:
        ...
        try:
            ...
            module_data = {
                ...
                "estimated_duration_minutes": training_content.get("estimated_duration_minutes", 15),
            ...
            logger.info(f"Assigned LineSmart training module {module_id} to technician {technician_id}")
            return module_id
    ...
    async def get_performance_improvement_metrics(self, technician_id: str, 
                                                timeframe_days: int = 180) -> Dict[str, Any]:
        """Calculate performance improvement metrics for ROI analysis"""
        try:
            firestore_manager = get_firestore_manager()
            
            cutoff_date = (datetime.now() - timedelta(days=timeframe_days)).isoformat()
            midpoint_date = (datetime.now() - timedelta(days=timeframe_days//2)).isoformat()
            ...
            work_orders = await firestore_manager.get_collection("work_orders", filters=filters)
            
            if len(work_orders) < 2:
                return {"error": "Insufficient data for performance analysis"}
            
            # Split into two periods for trend analysis
            first_half = [wo for wo in work_orders if wo.get("completed_date", "") < midpoint_date]
            second_half = [wo for wo in work_orders if wo.get("completed_date", "") >= midpoint_date]
            ...
            def calculate_period_metrics(work_orders):
                ...
                first_time_fixes = len([wo for wo in work_orders if wo.get("work_order_type", "") != "Corrective"])
                first_time_fix_rate = (first_time_fixes / len(work_orders) * 100) if work_orders else 0
            ...
            efficiency_improvement = (
                (first_period_metrics["average_hours_per_wo"] - second_period_metrics["average_hours_per_wo"]) 
                / first_period_metrics["average_hours_per_wo"] * 100
            ) if first_period_metrics["average_hours_per_wo"] > 0 else 0
            
            fix_rate_improvement = (
                second_period_metrics["first_time_fix_rate"] - first_period_metrics["first_time_fix_rate"]
            )
            ...
            return {
                ...
                "improvements": {
                    ...
                    "first_time_fix_improvement_percent": round(fix_rate_improvement, 1),
                    "work_order_volume_change": second_period_metrics["work_orders_completed"] - first_period_metrics["work_orders_completed"]
                },
                "roi_indicators": {
                    "estimated_time_savings_hours": (
                        first_period_metrics["average_hours_per_wo"] - second_period_metrics["average_hours_per_wo"]
                    ) * second_period_metrics["work_orders_completed"] if efficiency_improvement > 0 else 0,
                    "performance_trend": "improving" if efficiency_improvement > 5 or fix_rate_improvement > 5 else "stable"
                },
                "generated_at": datetime.now().isoformat()
            }
...
async def get_workforce_intelligence_dashboard(timeframe_days: int = 90) -> Dict[str, Any]:
    """Get comprehensive workforce intelligence dashboard data"""
    try:
        # Get overall skill gap analytics
        analytics = await linesmart_intelligence.get_skill_gap_analytics(timeframe_days=timeframe_days)
        
        # Get performance metrics for Genesis project technicians
        jake_metrics = await linesmart_intelligence.get_performance_improvement_metrics("4", timeframe_days)  # Jake Thompson
        anna_metrics = await linesmart_intelligence.get_performance_improvement_metrics("5", timeframe_days)  # Anna Kowalski
        ...
        return {
            ...
            "overview": {
                ...
                "training_modules_generated": len(analytics.get("training_focus_areas", {}))
            },
