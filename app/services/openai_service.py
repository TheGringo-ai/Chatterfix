...
class OpenAIService:
    ...
    async def generate_response(self, prompt: str, context: str = "", user_id: Optional[int] = None) -> str:
        ...
        try:
            ...
            response = client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "You are ChatterFix, an intelligent CMMS assistant for industrial maintenance technicians. You help manage work orders, find parts, and troubleshoot equipment efficiently."},
    ...
    async def run_assistant_agent(self, message: str, context: str, user_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        client = self._get_client(user_id)
        if not client:
            return {"response": "AI Service is unavailable. Please configure your OpenAI API Key in Settings."}
    ...
    async def _create_asset(self, name: str, type: str, location: str, status: str = "Active"):
        from app.core.database import get_db_connection
        conn = get_db_connection()
        try:
            cursor = conn.execute("""
