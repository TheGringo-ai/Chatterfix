...
import math
import random
from app.services.advanced_scheduler_service import ScheduleType, Priority, AssetRequirement
...
class PMAutomationEngine:
    ...
    async def generate_pm_schedule(self, start_date: datetime, 
                                 end_date: datetime) -> List[GeneratedWorkOrder]:
        ...
        condition_orders = await self._check_condition_based_triggers(start_date, end_date)
        ...
        logger.info(f"Generated {len(generated_orders)} PM work orders for period {start_date} to {end_date}")
        
        return generated_orders
    
    async def _generate_orders_for_rule(self, rule: MaintenanceScheduleRule,
                                      start_date: datetime, 
                                      end_date: datetime) -> List[GeneratedWorkOrder]:
        """Generate work orders for a specific schedule rule"""
        orders = []
        
        if rule.schedule_type in [ScheduleType.CONDITION_BASED, ScheduleType.USAGE_BASED]:
            return orders  # These are handled separately
        
        template = self.maintenance_templates.get(rule.template_id)
        if not template:
            logger.warning(f"Template {rule.template_id} not found for rule {rule.rule_id}")
            return orders
    ...
    async def _check_condition_based_triggers(self, start_date: datetime,
                                            end_date: datetime) -> List[GeneratedWorkOrder]:
            ...
            for meter in meters:
                ...
                for template_id in triggered_templates:
                    ...
                    work_order = GeneratedWorkOrder(
                        ...
                        due_date=datetime.now() + timedelta(hours=24),  # Urgent response
                        estimated_duration=template.estimated_duration,
                        required_skills=template.required_skills.copy(),
                        required_parts=template.required_parts.copy(),
                        required_tools=template.required_tools.copy(),
    ...
    async def _check_usage_based_triggers(self, start_date: datetime,
                                        end_date: datetime) -> List[GeneratedWorkOrder]:
    ...
    async def _simulate_meter_readings(self, meter: AssetMeter):
        ...
        if days_since_reading >= meter.reading_frequency:
            ...
            elif meter.meter_type in ["production_cycles", "operation_cycles", "runtime_hours"]:
    ...
    def _meter_matches_trigger(self, meter: AssetMeter, trigger: MaintenanceTrigger) -> bool:
    ...
    def _calculate_priority(self, template: MaintenanceTemplate, due_date: datetime) -> Priority:
    ...
    def _apply_seasonal_adjustments(self, orders: List[GeneratedWorkOrder],
    ...
    async def get_pm_schedule_overview(self, days_ahead: int = 30) -> Dict[str, Any]:
        ...
        return {
            ...
            } for order in upcoming_orders[:20]],  # Limit to first 20 for display
            "templates_summary": [{
    ...
    async def update_meter_reading(self, meter_id: str, new_value: float) -> Dict[str, Any]:
        ...
        for template_id in triggered_templates:
            ...
            if template:
                ...
                work_order = GeneratedWorkOrder(
                    ...
                    due_date=datetime.now() + timedelta(hours=4),  # 4 hour response time
                    estimated_duration=template.estimated_duration,
                    required_skills=template.required_skills.copy(),
                    required_parts=template.required_parts.copy(),
                    required_tools=template.required_tools.copy(),
