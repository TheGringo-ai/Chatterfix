...
async def process_voice_command(voice_text: str, technician_id: Optional[int] = None):
    ...
    try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.warning(f"Grok AI returned status {response.status_code}")
        ...
        priority = "Medium"
        if any(word in voice_text.lower() for word in ["urgent", "emergency", "critical", "broken"]):
            priority = "High"
        elif any(word in voice_text.lower() for word in ["routine", "scheduled", "minor"]):
            priority = "Low"
