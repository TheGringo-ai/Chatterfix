...
try:
    ...
    logging.warning("google-cloud-secret-manager not installed. Cannot fetch secrets from Secret Manager.")
...
def _get_secret_sync_from_manager(secret_id: str, project_id: Optional[str] = None) -> Optional[str]:
    ...
    try:
        ...
        if not project_id:
            logger.warning(f"GOOGLE_CLOUD_PROJECT environment variable not set, cannot fetch secret '{secret_id}' from Secret Manager.")
            return None
...
async def get_secret(secret_id: str, project_id: Optional[str] = None) -> Optional[str]:
    ...
    env_value = os.getenv(secret_id.upper()) # Assuming ENV var name matches secret_id in uppercase
    if env_value:
        return env_value
    ...
    if SECRET_MANAGER_AVAILABLE:
        ...
        return await asyncio.to_thread(_get_secret_sync_from_manager, secret_id, project_id)
    
    logger.warning(f"Secret '{secret_id}' not found in environment or Secret Manager (or client not available).")
    return None
