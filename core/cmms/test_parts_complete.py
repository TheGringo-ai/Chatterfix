...
def test_api_endpoints():
    ...
    print(f"     ✅ Status: {response.status_code}, Count: {results['list_parts']['count']}")
    ...
    if test_part_id:
        ...
        print(f"     ✅ Status: {response.status_code}, Name: {results['get_part']['part_name']}")
    ...
    if test_part_id:
        ...
        print(f"     ✅ Status: {response.status_code}, New Qty: {results['update_part']['updated_quantity']}")
    ...
    if test_part_id:
        ...
        print(f"     ✅ Status: {response.status_code}, Remaining: {results['issue_part']['remaining_qty']}")
    ...
    if test_part_id:
        ...
        print(f"     ✅ Status: {response.status_code}, Success: {results['delete_part']['success']}")
...
def generate_test_report(api_results, ui_results, error_results):
    ...
    for test_name, result in api_results.items():
        ...
        print(f"   • {test_name.replace('_', ' ').title()}: {status} ({result.get('status_code', 'N/A')})")
    print()
    
    # UI Tests Summary
    print("🖥️ USER INTERFACE TEST RESULTS:")
    ui_passed = sum(1 for key, val in ui_results['dashboard_load'].items() 
                   if key != 'status_code' and val)
    ui_total = len([k for k in ui_results['dashboard_load'].keys() if k != 'status_code'])
    ...
    error_passed = sum(1 for test in error_results.values() 
    ...
    print("   ✅ Real-time Stats - Live calculation of totals, low stock, critical alerts")
