...
echo "🎯 Final ChatterFix Backend Deployment"
echo "======================================"
...
# Create final startup script that definitely works
cat > final-startup.sh << 'FINAL_EOF'
#!/bin/bash
set -e
...
gcloud compute instances add-metadata $VM_NAME \
    --zone=$ZONE \
    --project=$PROJECT \
    --metadata-from-file startup-script=final-startup.sh

echo "🔄 Restarting VM..."
gcloud compute instances reset $VM_NAME --zone=$ZONE --project=$PROJECT

echo ""
echo "⏳ Waiting for final deployment (120 seconds)..."
sleep 120
...
curl -s http://35.237.149.25:8081/api/work-orders

echo ""
echo "🎉 FINAL DEPLOYMENT COMPLETE!"
echo "============================="
echo "✅ ChatterFix Backend: http://35.237.149.25:8081"
