...
echo "🔥 FORCE COMPLETE CHATTERFIX DEPLOYMENT"
echo "========================================"
...
# Create aggressive replacement startup script
cat > force-startup.sh << 'FORCE_EOF'
#!/bin/bash
set -e
...
chmod +x force-startup.sh

# Deploy the force startup script
echo "📤 Deploying force startup script..."
gcloud compute instances add-metadata $VM_NAME \
    --zone=$ZONE \
    --project=$PROJECT \
    --metadata-from-file startup-script=force-startup.sh

echo "🔄 Force restarting VM..."
gcloud compute instances reset $VM_NAME --zone=$ZONE --project=$PROJECT

echo ""
echo "⏳ Waiting for complete deployment..."
sleep 60

echo "🩺 Testing complete deployment..."
if curl -s http://35.237.149.25:8080/health | grep "ChatterFix CMMS Complete" > /dev/null; then
    echo "✅ SUCCESS! ChatterFix Complete is running!"
else
