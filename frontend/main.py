#!/usr/bin/env python3
"""
🚀 ChatterFix CMMS - Phase 7 Unified Gateway
Serves the complete enterprise frontend with all Phase 7 services integrated
"""
...
app = FastAPI(title="ChatterFix CMMS Phase 7 Gateway", version="7.0.0")
...
async def health_check_all():
    ...
    health_status["overall"] = "healthy" if not unhealthy_services else "degraded"
    
    # Enhanced service information
    health_status["enhanced_services"] = {
        "work_orders": {
            "status": health_status["services"].get("work_orders", "unknown"),
            "features": ["crud", "attachments", "exports", "voice_ai"]
        },
        "assets": {
            "status": health_status["services"].get("assets", "unknown"),
            "features": ["crud", "attachments", "exports", "maintenance_tracking"]
        },
        "parts": {
            "status": health_status["services"].get("parts", "unknown"),
            "features": ["crud", "checkout", "inventory_tracking", "barcode", "exports"]
        },
        "voice_ai": {
            "status": health_status["services"].get("voice_ai", "unknown"),
...
async def suppliers_proxy(request: Request):
    """Proxy to Suppliers endpoint"""
    response = await proxy_request(SERVICES["parts"], "suppliers", request)
    return response.json() if response.headers.get("content-type", "").startswith("application/json") else response.text

@app.api_route("/api/suppliers/{supplier_id}", methods=["GET"])
async def supplier_detail_proxy(supplier_id: int, request: Request):
    """Proxy to Supplier detail endpoint"""
    response = await proxy_request(SERVICES["parts"], f"suppliers/{supplier_id}", request)
    return response.json() if response.headers.get("content-type", "").startswith("application/json") else response.text

# Voice AI API routes
@app.api_route("/api/voice/intent", methods=["POST"])
async def voice_intent_proxy(request: Request):
    """Proxy to Voice AI intent processing endpoint"""
    response = await proxy_request(SERVICES["voice_ai"], "voice/intent", request)
    return response.json() if response.headers.get("content-type", "").startswith("application/json") else response.text

@app.api_route("/api/voice/test", methods=["POST"])
async def voice_test_proxy(request: Request):
    """Proxy to Voice AI test endpoint"""
    response = await proxy_request(SERVICES["voice_ai"], "voice/test", request)
    return response.json() if response.headers.get("content-type", "").startswith("application/json") else response.text

@app.api_route("/api/voice/intents", methods=["GET"])
async def voice_intents_proxy(request: Request):
    """Proxy to Voice AI supported intents endpoint"""
    response = await proxy_request(SERVICES["voice_ai"], "voice/intents", request)
    return response.json() if response.headers.get("content-type", "").startswith("application/json") else response.text
...
async def root():
    """Serve the enhanced ChatterFix CMMS Phase 7 interface"""
    return """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>ChatterFix CMMS - Phase 7 Enterprise Platform</title>
...
<body>
    <div id="loadingOverlay" class="loading-overlay">
        <div class="text-center">
            ...
            <div class="mt-3">Loading Phase 7 Enterprise Platform...</div>
        </div>
    </div>
                ...
                <div class="p-4">
                    <div class="d-flex align-items-center mb-4">
                        ...
                        <div>
                            ...
                            <small class="enterprise-badge">Phase 7</small>
                        </div>
                    </div>
                    ...
                    <div class="container-fluid p-4">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <div>
                                ...
                                <p class="text-muted">Phase 7 - Complete CMMS with enhanced AI and voice capabilities</p>
                            </div>
                ...
                <div class="p-3">
                    ...
                    <div id="chatMessages" style="height: 200px; overflow-y: auto; border: 1px solid #eee; padding: 10px; margin-bottom: 10px;">
                        <div class="text-muted text-center py-3">
                            ...
                            <br>Hi! I'm Fix It Fred, your enhanced AI assistant with Phase 7 capabilities including advanced CMMS, voice AI, and comprehensive enterprise features.
                        </div>
                    </div>
    ...
    <script>
                            ...
                            ${workOrders.map(wo => `
                                <tr style="cursor: pointer;" onclick="viewWorkOrder(${wo.id})" onmouseover="this.style.backgroundColor='#f8f9fa'" onmouseout="this.style.backgroundColor=''">
                                    ...
                                    <td onclick="event.stopPropagation();">
                                        ...
                                        <button class="btn btn-sm btn-outline-secondary" onclick="editWorkOrder(${wo.id})" title="Edit">
            ...
            return `
                <div class="table-responsive">
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                            ${assets.map(asset => `
                                <tr style="cursor: pointer;" onclick="viewAsset(${asset.id})" onmouseover="this.style.backgroundColor='#f8f9fa'" onmouseout="this.style.backgroundColor=''">
                                    <td>#${asset.id}</td>
                                    ...
                                    <td onclick="event.stopPropagation();">
                                        <button class="btn btn-sm btn-outline-primary me-1" onclick="viewAsset(${asset.id})" title="View Details">
                                            <i class="fas fa-eye"></i>
                                        </button>
                                        <button class="btn btn-sm btn-outline-secondary" onclick="editAsset(${asset.id})" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                    </td>
                                </tr>
            ...
            return `
                <div class="table-responsive">
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                            ${parts.map(part => `
                                <tr class="${part.quantity_on_hand <= part.minimum_stock_level ? 'table-warning' : ''}" style="cursor: pointer;" onclick="viewPart(${part.id})" onmouseover="this.style.backgroundColor='#f8f9fa'" onmouseout="this.style.backgroundColor=''">
                                    <td>#${part.id}</td>
                                    ...
                                    <td onclick="event.stopPropagation();">
                                        <button class="btn btn-sm btn-outline-primary me-1" onclick="viewPart(${part.id})" title="View Details">
                                            <i class="fas fa-eye"></i>
                                        </button>
                                        <button class="btn btn-sm btn-outline-secondary" onclick="editPart(${part.id})" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                    </td>
                                </tr>
        ...
        async function sendChatMessage() {
            ...
            addChatMessage('assistant', 'Analyzing with Phase 7 enterprise capabilities...');
            ...
            try {
                ...
                const messages = document.getElementById('chatMessages');
                messages.removeChild(messages.lastChild);
                addChatMessage('assistant', `I understand you're asking about "${message}". The enhanced AI service is currently starting up. In the meantime, I can help with general CMMS questions. The Phase 7 platform includes advanced work order management, asset tracking, parts inventory, voice AI commands, and comprehensive enterprise analytics.`);
            }
        }
        ...
        setTimeout(() => {
            addChatMessage('assistant', 'Phase 7 Enterprise Platform loaded! I can help with work orders, asset management, parts inventory, troubleshooting, voice commands, and comprehensive CMMS functions. What would you like to know?');
        }, 2000);
