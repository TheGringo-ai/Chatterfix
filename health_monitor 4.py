...
class HealthMonitor:
    ...
    async def init_database(self):
        ...
        async with aiosqlite.connect(self.db_path) as conn:
            ...
            await conn.execute('CREATE INDEX IF NOT EXISTS idx_metrics_timestamp ON health_metrics(timestamp)')
            await conn.execute('CREATE INDEX IF NOT EXISTS idx_incidents_timestamp ON incidents(timestamp)')
            await conn.execute('CREATE INDEX IF NOT EXISTS idx_slo_timestamp ON slo_events(timestamp)')
    ...
    async def check_system_resources(self) -> Dict[str, Any]:
        ...
        overall_status = max([m.status for m in metrics.values()], 
    ...
    async def check_disk_space(self) -> Dict[str, Any]:
        ...
        for partition in psutil.disk_partitions():
            try:
                ...
                logger.warning(f"Could not check disk usage for {partition.mountpoint}: {e}")
    ...
    async def check_memory_usage(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            'swap_usage': HealthMetric(
                'swap_usage_percent',
                swap.percent,
                'critical' if swap.percent > 50 else 'warning' if swap.percent > 25 else 'healthy'
            )
        }
    ...
    async def check_process_health(self) -> Dict[str, Any]:
        ...
        for proc_name in critical_processes:
            processes = [p for p in psutil.process_iter(['name', 'pid', 'cpu_percent', 'memory_percent']) 
    ...
    async def check_database_connection(self) -> Dict[str, Any]:
        ...
        try:
            async with aiosqlite.connect(self.db_path) as conn:
                ...
                await cursor.execute("SELECT COUNT(*) FROM health_metrics WHERE timestamp > ?", 
            ...
            return {
                'status': 'healthy',
                'metrics': {
                    'connection': HealthMetric('database_connection', 'connected', 'healthy'),
                    'recent_metrics': HealthMetric('recent_metrics_count', recent_count, 'healthy')
                },
            ...
            return {
                'status': 'critical',
                'metrics': {
                    'connection': HealthMetric('database_connection', f'error: {str(e)}', 'critical')
                },
    ...
    async def check_ai_services(self) -> Dict[str, Any]:
        ...
        try:
            ...
            ai_metrics['llama_available'] = HealthMetric('llama_model', 'simulated_healthy', 'healthy')
            ai_metrics['voice_processing'] = HealthMetric('voice_processor', 'available', 'healthy')
            ...
            return {
                'status': 'warning',
                'metrics': {
                    'ai_services': HealthMetric('ai_services', f'degraded: {str(e)}', 'warning')
                },
    ...
    async def check_external_dependencies(self) -> Dict[str, Any]:
        ...
        return {
            'status': 'healthy',
            'metrics': {
                'external_apis': HealthMetric('external_connectivity', 'available', 'healthy')
            },
    ...
    async def check_slo_compliance(self) -> Dict[str, Any]:
        ...
        for slo_name, slo in self.slos.items():
            ...
            since_time = datetime.now(timezone.utc) - timedelta(hours=slo.time_window_hours)
            ...
            budget_consumed_percent = (total_violation_minutes / slo.error_budget_minutes) * 100
            remaining_budget = max(0, slo.error_budget_minutes - total_violation_minutes)
                ...
                for metric_name, metric_data in check_result.get('metrics', {}).items():
                    if isinstance(metric_data, dict) and 'value' in metric_data:
                        ...
                        ''', (
                            ...
                            check_name, 
                            metric_name,
                            metric_data['value'] if isinstance(metric_data['value'], (int, float)) else None,
                            metric_data['status'],
    ...
    async def record_incident(self, incident_type: str, severity: str, description: str) -> int:
        """Record a new incident"""
        async with aiosqlite.connect(self.db_path) as conn:
            cursor = await conn.execute('''
                INSERT INTO incidents (timestamp, type, severity, description)
                VALUES (?, ?, ?, ?)
            ''', (datetime.now(timezone.utc).isoformat(), incident_type, severity, description))
    ...
    async def record_slo_violation(self, slo_name: str, duration_minutes: float, details: str = None):
        """Record an SLO violation"""
        async with aiosqlite.connect(self.db_path) as conn:
            await conn.execute('''
                INSERT INTO slo_events (timestamp, slo_name, event_type, duration_minutes, details)
                VALUES (?, ?, 'violation', ?, ?)
            ''', (datetime.now(timezone.utc).isoformat(), slo_name, duration_minutes, details))
            await conn.commit()
    ...
    async def get_health_history(self, hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            'metrics_summary': [dict(zip([col[0] for col in cursor.description], row)) for row in metrics_history],
            'recent_incidents': [dict(zip([col[0] for col in cursor.description], row)) for row in incidents],
            'slo_events': [dict(zip([col[0] for col in cursor.description], row)) for row in slo_events]
        }
...
async def get_readiness_probe() -> Dict[str, Any]:
    ...
    try:
        ...
        ready = (
            cpu_percent < 95 and  # Not completely overloaded
            memory.percent < 95   # Not out of memory
        )
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        for slo_name, slo_data in health_status['slo_status'].items():
            print(f"  {slo_name}: {slo_data['status']} ({slo_data['budget_consumed_percent']:.1f}% budget used)")
