...
echo "🐘 ChatterFix PostgreSQL Backend Deployment"
echo "==========================================="
echo "Enterprise-grade backend with PostgreSQL"
echo ""
...
# Create PostgreSQL deployment startup script
cat > postgresql-startup.sh << 'POSTGRES_STARTUP_EOF'
#!/bin/bash
set -e
...
async def test_connection():
    try:
        conn = await asyncpg.connect(
            'postgresql://postgres:REDACTED_DB_PASSWORD@35.225.244.14:5432/chatterfix_cmms'
        )
...
DATABASE_URL = "postgresql://postgres:REDACTED_DB_PASSWORD@35.225.244.14:5432/chatterfix_cmms"
pool = None
...
POSTGRES_STARTUP_EOF

chmod +x postgresql-startup.sh

# Deploy to VM
echo "📤 Deploying PostgreSQL backend to VM..."
gcloud compute instances add-metadata $VM_NAME \
    --zone=$ZONE \
    --project=$PROJECT \
    --metadata-from-file startup-script=postgresql-startup.sh

echo "🔄 Restarting VM with PostgreSQL backend..."
gcloud compute instances reset $VM_NAME --zone=$ZONE --project=$PROJECT

echo ""
echo "⏳ Waiting for PostgreSQL backend deployment..."
sleep 60

echo "🧪 Testing PostgreSQL deployment..."
echo "Health Check:"
curl -s http://35.237.149.25:8081/health | python3 -c "
import sys, json
try:
...
curl -s http://35.237.149.25:8081/api/work-orders | python3 -c "
import sys, json
try:
    data = json.load(sys.stdin)
    print(f'✅ Work Orders: {data.get(\"count\", 0)} items')
    print(f'✅ Database: {data.get(\"database\", \"unknown\")}')
except:
    print('❌ Work Orders API failed')
"

echo ""
echo "🎉 POSTGRESQL BACKEND DEPLOYMENT COMPLETE!"
echo "=========================================="
echo "✅ Enterprise-grade PostgreSQL backend deployed"
echo "✅ No SQLite dependencies"
echo "✅ Full CRUD operations"
echo "✅ Production-ready database"
echo ""
echo "🌐 Your ChatterFix now has:"
echo "  UI Gateway: http://35.237.149.25:8080"
echo "  PostgreSQL Backend: http://35.237.149.25:8081"
echo "  Database: PostgreSQL @ 35.225.244.14"
echo "  AI: Ollama integration ready"