...
class DependencyAnalyzer:
    def __init__(self, project_root: str):
        ...
        self.stdlib_patterns = {
            'os', 'sys', 'json', 're', 'time', 'datetime', 'logging', 'pathlib',
            'collections', 'typing', 'functools', 'itertools', 'asyncio', 'contextlib',
    ...
    def map_import_to_package(self, import_name: str) -> Optional[str]:
        ...
        all_packages = set(self.requirements_deps.keys()) | set(self.dev_requirements_deps.keys()) | set(self.ai_service_deps.keys())
    ...
    def find_missing_dependencies(self) -> Set[str]:
        ...
        for import_name in third_party_imports:
            ...
            if not mapped_package:
                ...
                found = (mapped_package in self.requirements_deps or 
                        mapped_package in self.dev_requirements_deps or
                        mapped_package in self.ai_service_deps)
    ...
    def analyze(self) -> Dict:
        ...
        self.requirements_deps = self.parse_requirements_file(self.project_root / 'requirements.txt')
        self.dev_requirements_deps = self.parse_requirements_file(self.project_root / 'requirements-dev.txt')
        self.ai_service_deps = self.parse_requirements_file(self.project_root / 'ai-team-service' / 'requirements.txt')
        ...
        return {
            'summary': {
                ...
                'total_dependencies': len(self.requirements_deps) + len(self.dev_requirements_deps) + len(self.ai_service_deps),
                'unused_dependencies': len(unused_deps),
                'missing_dependencies': len(missing_deps),
                'duplicate_dependencies': len(duplicate_deps)
            },
            'import_categories': {k: sorted(list(v)) for k, v in import_categories.items()},
            'dependencies': {
            ...
            'recommendations': self._generate_recommendations(unused_deps, missing_deps, duplicate_deps, import_categories)
        }

    def _generate_recommendations(self, unused_deps: Dict, missing_deps: Set, 
                                duplicate_deps: Dict, import_categories: Dict) -> Dict:
        ...
        if unused_deps:
            recommendations['cleanup'].append(f"Remove {len(unused_deps)} unused dependencies to reduce bundle size")
        ...
        if missing_deps:
            recommendations['additions'].append(f"Add {len(missing_deps)} missing dependencies")
            for dep in list(missing_deps)[:5]:  # Show first 5
                recommendations['additions'].append(f"  - {dep} (imported but not in requirements)")
        
        # Organization recommendations
        if duplicate_deps:
            recommendations['organization'].append(f"Resolve {len(duplicate_deps)} duplicate dependencies")
            for dep, files in duplicate_deps.items():
                recommendations['organization'].append(f"  - {dep} in {', '.join(files)}")
        
        # Security recommendations
        security_packages = ['cryptography', 'requests', 'urllib3', 'pillow']
        for pkg in security_packages:
            if pkg in self.requirements_deps:
                recommendations['security'].append(f"Monitor {pkg} for security updates")
