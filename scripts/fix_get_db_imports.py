...
def find_insert_index(lines):
    ...
    while i < n and (
        lines[i].startswith("#!") or re.match(r"#\s*-*\s*coding", lines[i])
    ):
        i += 1
    ...
    while i < n and (
        re.match(r"\s*(import|from)\s+", lines[i]) or lines[i].strip() == ""
    ):
        i += 1
...
def main():
    ...
    print(
        " - Open app/core/firestore_db.py and move module-level imports to the top of the file (E402)."
    )
    print(
        " - Remove unused imports / variables (F401 / F841). You can run autoflake to help remove unused imports:"
    )
    print("     pip install autoflake")
    print("     autoflake --remove-all-unused-imports --in-place -r app/")
    print(
        " - Fix the F541 missing-placeholder f-string in app/routers/demo.py at the indicated line (remove the leading 'f' if there are no placeholders)."
    )
    print(
        " - Add missing imports (e.g., timedelta) where reported (e.g., from datetime import timedelta)."
    )
    print(
        " - Remove trailing whitespace and blank-line whitespace (W291 / W293). Tools like ruff/black can help."
    )
    print("\nAfter making changes, re-run:")
    print("    flake8 app/ --max-line-length=100 --ignore=E203,W503 --count")
    print(
        "If you want, I can generate a PR with these automated import insertions — tell me and I'll create one."
    )
