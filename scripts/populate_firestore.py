...
"
    ...
    for user_data in users_to_create:
        try:
            ...
            print(
                f"  ✅ Created Firebase Auth user: {user_data['display_name']} ({user_data['role']})"
            )
            ...
            user_doc_data = {
                ...
                "permissions": get_permissions_for_role(
                    user_data["role"]
                ),  # Add permissions to user document
            }
            await firestore_manager.create_document(
                "users", user_doc_data, doc_id=user_record.uid
            )
    ...
    manager_uid = next(
        (u["uid"] for u in firestore_users if u["role"] == "manager"), None
    )
    technician_uids = [u["uid"] for u in firestore_users if u["role"] == "technician"]
    if not manager_uid or not technician_uids:
        print(
            "⚠️ Not enough users created. Please ensure Firebase Auth setup is correct."
        )
        return
    ...
    for _ in range(10):
        asset_type = random.choice(
            ["Pump", "Motor", "HVAC Unit", "Conveyor Belt", "Sensor Array"]
        )
        ...
        assets_to_create.append(
            {
                ...
                "next_maintenance_date": (
                    fake.date_this_year() + timedelta(days=random.randint(30, 365))
                ).isoformat(),
    ...
    if not created_assets or not firestore_users:
        ...
        for _ in range(20):
            ...
            work_order_data = {
                ...
                "work_order_type": random.choice(
                    ["Preventive", "Corrective", "Predictive", "Emergency"]
                ),
            }
            if status == "Completed":
                work_order_data["completed_date"] = (
                    due_date - timedelta(days=random.randint(0, 2))
                ).isoformat()
                work_order_data["actual_hours"] = work_order_data[
                    "estimated_hours"
            ...
            try:
                ...
                print(
                    f"  ✅ Created work order: {work_order_data['title']} (ID: {wo_id})"
                )
            except Exception as e:
                print(
                    f"  ❌ Failed to create work order {work_order_data['title']}: {e}"
                )
    ...
    for _ in range(5):
        ...
        skill_category = random.choice(
            ["Electrical", "Mechanical", "Safety", "Hydraulics"]
        )
    ...
    for module_data in training_modules_to_create:
        try:
            ...
            created_training_modules.append(
                {"id": module_id, "title": module_data["title"]}
            )
            print(
                f"  ✅ Created training module: {module_data['title']} (ID: {module_id})"
            )
        except Exception as e:
    ...
    if not created_training_modules or not firestore_users:
        print(
            "  ⚠️ Skipping user training assignment: No training modules or users available."
        )
    else:
        for user_item in firestore_users:
            if user_item["role"] == "technician":
                for _ in range(
                    random.randint(1, 3)
                ):  # Assign 1-3 modules per technician
                    ...
                    if status in ["in_progress", "completed"]:
                        started_date = fake.date_this_year() + timedelta(
                            days=random.randint(1, 10)
                        )
                        training_assignment_data["started_date"] = (
                            started_date.isoformat()
                        )
                    if status == "completed":
                        training_assignment_data["completed_date"] = (
                            started_date + timedelta(days=random.randint(1, 5))
                        ).isoformat()
                        training_assignment_data["score"] = random.randint(70, 100)

                    try:
                        assignment_id = await firestore_manager.create_document(
                            "user_training", training_assignment_data
                        )
                        print(
                            f"  ✅ Assigned '{module['title']}' to {user_item['full_name']} (Status: {status})"
                        )
                    except Exception as e:
                        print(
                            f"  ❌ Failed to assign training {module['title']} to {user_item['full_name']}: {e}"
                        )
