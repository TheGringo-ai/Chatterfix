...
class WorkflowHealthMonitor:
    ...
    def check_workflow_runs(self, days: int = 7) -> Dict:
        ...
        if analysis["total_runs"] > 0:
            analysis["success_rate"] = (
                analysis["successful"] / analysis["total_runs"]
            ) * 100
        else:
        ...
        for workflow_file in workflow_dir.glob("*.yml"):
            ...
            try:
                ...
                if "permissions" not in content:
                    issues.append(
                        f"{workflow_file.name}: Missing permissions (security best practice)"
                    )
                ...
                for job_name, job_config in content.get("jobs", {}).items():
                    if "timeout-minutes" not in job_config:
                        issues.append(
                            f"{workflow_file.name}: Job '{job_name}' missing timeout"
                        )
    ...
    def generate_health_report(self) -> Dict:
        ...
        report["status"] = (
            "healthy" if score >= 80 else "warning" if score >= 60 else "critical"
        )
    ...
    def print_report(self, report: Dict):
        ...
        status_icon = (
            ...
            else "⚠️" if report["status"] == "warning" else "❌"
        )
        ...
        if "error" not in runs:
            ...
            print(
                f"   Total: {runs['total_runs']} | Success: {runs['successful']} | Failed: {runs['failed']}"
            )
