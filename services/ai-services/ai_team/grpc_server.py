...
from .autogen_framework import get_orchestrator, CollaborationResult
...
class AIModel:
    def __init__(self, name, provider, capabilities, available=True, performance_score=0.8, version="1.0"):
        self.name = name
...
class ModelResponse:
    def __init__(self, model_name, response, confidence=0.8, response_time_ms=500, success=True, error_message=""):
...
class AITeamServiceImplementation:
    ...
    async def ExecuteTask(self, request: TaskRequest) -> TaskResponse:
        ...
        try:
            ...
            result = await self.orchestrator.execute_collaborative_task(
                ...
                required_agents=list(request.required_models) if request.required_models else None,
            ...
            for agent_resp in result.agent_responses:
                model_resp = ModelResponse(
                    ...
                    response_time_ms=int(result.total_time * 1000 / len(result.agent_responses)),
                    success=True
                )
    ...
    async def HealthCheck(self, request: HealthRequest) -> HealthResponse:
        ...
        try:
            ...
            response.active_models = [agent["name"] for agent in agent_status["agents"] if agent["enabled"]]
