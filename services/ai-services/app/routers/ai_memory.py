...
from app.services.ai_memory_system import get_ai_memory, AIMemorySystem
...
async def record_integration_error(
    request: IntegrationErrorRequest,
...
async def check_integration_issues(
    request: IntegrationCheckRequest,
    memory: AIMemorySystem = Depends(get_ai_memory)
):
...
async def get_success_patterns(
    ...
    try:
        if pattern_type:
            pattern = await memory.get_success_pattern(pattern_type, source_service, target_service)
            if pattern:
                return {
                    "pattern_found": True,
                ...
                return {"pattern_found": False, "message": "No pattern found for specified criteria"}
        else:
            # Return all patterns for this service pair
            all_patterns = [
                pattern.__dict__ for pattern in memory.success_patterns.values()
                if pattern.source_service == source_service and pattern.target_service == target_service
            ]
            return {
    ...
    try:
        ...
        return {
            ...
            "insights": {
                ...
                "avg_resolution_time": sum(e.resolution_time_minutes for e in history) / len(history) if history else 0
            }
        }
    ...
    try:
        ...
        stats.update({
            ...
            "memory_effectiveness": {
                ...
                "time_saved_minutes": sum(e.resolution_time_minutes for e in memory.integration_errors.values()),
...
async def export_memory(memory: AIMemorySystem = Depends(get_ai_memory)):
    ...
    try:
        export_data = {
            ...
            "integration_errors": {k: v.__dict__ for k, v in memory.integration_errors.items()},
            "success_patterns": {k: v.__dict__ for k, v in memory.success_patterns.items()},
            "export_metadata": {
                "export_time": memory.get_memory_stats()["last_updated"],
                "total_items": len(memory.api_contracts) + len(memory.integration_errors) + len(memory.success_patterns)
            }
        }
