...
class AIServicesClient:
    ...
    async def process_unified_request(self, request_data: dict):
        ...
        try:
            response = await self.client.post(
                f"{self.ai_services_url}/unified-ai/process",
                json=request_data
            )
            return response.json() if response.status_code == 200 else None
        except Exception as e:
    ...
    async def analyze_maintenance_issue(self, issue_data: dict):
        ...
        try:
            ...
            response = await self.client.post(
                f"{self.ai_services_url}/fix-it-fred/analyze",
                json=fix_it_fred_request
            )
            return response.json() if response.status_code == 200 else None
        except Exception as e:
    ...
    try:
        ...
        return {
            ...
            "ai_services": {
                ...
                "status": ai_health.get("service", "unknown") if ai_health else "disconnected"
            },
            "business_features": [
                "work_orders", "assets", "inventory", "team_management",
                "preventive_maintenance", "dashboard", "analytics", "ai_integration"
            ]
        }
...
async def cmms_service_info():
    ...
    return {
        ...
        "ai_integration": {
            "status": "connected" if ai_health and ai_health.get("healthy") else "disconnected",
            "ai_services_url": os.getenv("AI_SERVICES_URL", "http://localhost:8002"),
            "endpoints": ["/ai/*"],
            "capabilities": ["intelligent_analysis", "autonomous_repair", "predictive_maintenance"]
        }
    }
