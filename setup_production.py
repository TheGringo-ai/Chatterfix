...
def check_gcp_environment():
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout:
            print(f"🌐 Running on GCP - Project: {result.stdout}")
...
def setup_gcp_environment():
    ...
    print("🔧 Configuring for GCP deployment...")
...
"""
    ...
    print("✅ Production environment configured (.env.production)")
    return True
    ...
    try:
        ...
        if creds_path and os.path.exists(creds_path):
            ...
            try:
                ...
                print("✅ Firebase connection successful (Application Default Credentials)!")
                return True
            except Exception as e:
                print(f"❌ Firebase ADC test failed: {e}")
                return False
...
def main():
    """Main setup function"""
    print("🚀 ChatterFix Production Setup")
    print("=" * 50)
    ...
    if not install_firebase_deps():
        print("❌ Failed to install Firebase dependencies")
        return False
    
    # Setup environment
    if is_gcp:
        if not setup_gcp_environment():
            print("❌ GCP environment setup failed")
            return False
    else:
        if not setup_local_development():
            print("❌ Local development setup failed")
            return False
    
    # Test connection
    if test_firebase_connection():
        print("\n🎉 Setup Complete!")
        print("✅ Firebase configured and tested")
        print("🔥 ChatterFix ready for production on GCP")
        ...
        print("\n❌ Setup incomplete - Firebase connection failed")
        return False
