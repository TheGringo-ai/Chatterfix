...
def test_database_operations():
    ...
    try:
        ...
        if len(users) > 0:
            print(
                "✅ Database operations working: Created table, inserted data, retrieved data"
            )
            return True
        else:
    ...
    for file_path in static_files:
        if os.path.exists(file_path):
            try:
                ...
                if content.strip():
                    print(
                        f"✅ Static file {file_path}: Valid content ({len(content)} chars)"
                    )
                    success_count += 1
                else:
        ...
        if os.path.exists(template_path):
            try:
                ...
                if content.strip() and "<html>" in content.lower():
                    ...
                    print(
                        f"⚠️ Template {template_path}: Has content but may not be complete HTML"
                    )
                    success_count += 1
                else:
