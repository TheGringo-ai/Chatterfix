...
def test_security_patterns():
    ...
    if security_issues:
        ...
        for issue in security_issues[:5]:  # Show first 5 issues
            print(f"   - {issue['file']}: {issue['issue']} ({issue['matches']} matches)")
        if len(security_issues) > 5:
            print(f"   ... and {len(security_issues) - 5} more")
        return False
    else:
        print(f"✅ No obvious security issues found in {total_files_checked} Python files")
        return True
...
def test_code_quality_metrics():
    ...
    return len(long_files) < len(python_files) * 0.2  # Less than 20% of files are too long
...
def main():
    ...
    print("-"*60)
    print(f"Total: {total_tests} | Passed: {passed_tests} | Need Review: {total_tests - passed_tests}")
