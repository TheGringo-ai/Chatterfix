...
class SignupFirebaseTest:
    ...
    def test_signup_route_structure(self):
        ...
        try:
            ...
            if missing:
                ...
                self.log_result(
                    "Signup Router Elements", True, "All required elements present"
                )
    ...
    def test_firebase_auth_structure(self):
        ...
        try:
            ...
            if missing:
                self.log_result(
                    "Firebase Auth Methods", False, f"Missing methods: {missing}"
                )
            else:
                self.log_result(
                    "Firebase Auth Methods", True, "All required methods present"
                )

            # Check for proper error handling
            if "try:" in content and "except" in content and "logger" in content:
                self.log_result(
                    "Firebase Error Handling", True, "Proper error handling implemented"
                )
            else:
                self.log_result(
    ...
    def test_signup_template(self):
        ...
        try:
            ...
            missing_fields = [
                field for field in required_form_fields if field not in content
            ]
            missing_security = [
            ...
            if missing_fields:
                self.log_result(
                    "Signup Form Fields", False, f"Missing: {missing_fields}"
                )
            else:
                self.log_result(
                    "Signup Form Fields", True, "All required fields present"
                )

            if missing_security:
                self.log_result(
                    "Signup Security Features", False, f"Missing: {missing_security}"
                )
            else:
                self.log_result(
                    "Signup Security Features", True, "Security features implemented"
                )
    ...
    def test_environment_configuration(self):
        ...
        firebase_vars = ["USE_FIRESTORE", "GOOGLE_CLOUD_PROJECT", "FIREBASE_API_KEY"]
        ...
        if configured_vars:
            self.log_result(
                "Firebase Environment", True, f"Configured: {configured_vars}"
            )
        else:
            self.log_result(
                "Firebase Environment", False, f"Missing all vars: {missing_vars}"
            )
        ...
        if os.path.exists(".env"):
            try:
                ...
                if any(var in env_content for var in firebase_vars):
                    ...
                    self.log_result(
                        "Environment File", False, "No Firebase vars in .env"
                    )
            except Exception as e:
            ...
            if result.returncode == 0:
                ...
                if "Firebase service created successfully" in output:
                    ...
                    if "Firebase available: False" in output:
                        self.log_result(
                            "Firebase Fallback", True, "Graceful fallback to SQLite"
                        )
                    elif "Firebase available: True" in output:
                        self.log_result(
                            "Firebase Connection", True, "Firebase properly connected"
                        )
                    else:
                        self.log_result(
                            "Firebase Status", False, "Could not determine status"
                        )
                else:
                    self.log_result(
                        "Firebase Service Initialization",
                ...
                self.log_result(
                    "Firebase Service Test", False, f"Error: {result.stderr}"
                )
...
"""
    ...
    def test_signup_endpoints(self):
        ...
        for endpoint, description in endpoints:
            try:
                ...
                if response.status_code == 200:
                    self.log_result(
                        f"GET {endpoint}", True, f"Status: {response.status_code}"
                    )
                else:
                    self.log_result(
                        f"GET {endpoint}", False, f"Status: {response.status_code}"
                    )
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/signup", data=signup_data, timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                if result.get("success"):
                    ...
                    self.log_result(
                        "POST /signup",
                        False,
                        f"Signup failed: {result.get('error', 'Unknown error')}",
                    )
            else:
                self.log_result(
                    "POST /signup", False, f"Status: {response.status_code}"
                )
    ...
    def test_password_validation(self):
        ...
        for case in test_cases:
            try:
                ...
                response = requests.post(
                    f"{self.base_url}/signup", data=signup_data, timeout=5
                )
    ...
    def print_summary(self):
        ...
        print("\n💡 RECOMMENDATIONS:")
        firebase_available = any(
            "Firebase Connection" in r["test"] and r["success"]
        ...
        signup_working = any(
            "POST /signup" in r["test"] and r["success"] for r in self.test_results
        )
        if signup_working:
