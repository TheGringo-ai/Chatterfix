        ...
        test_work_orders = [
            {
                ...
                "completion_parts": [
                    {
                        "part_id": "seal_001",
                        "part_name": "Hydraulic Cylinder Seal",
                        "quantity_used": 1,
                        "unit_cost": 45.00,
                    }
                ],
...
async def run_intelligence_analysis(work_order_ids):
    ...
    for wo_id in work_order_ids:
        try:
            ...
            print(
                f"   • Failure Type: {analysis.get('failure_analysis', {}).get('failure_type', 'N/A')}"
            )
            print(
                f"   • Root Cause: {analysis.get('failure_analysis', {}).get('root_cause_category', 'N/A')}"
            )
            print(
                f"   • Preventability Score: {analysis.get('failure_analysis', {}).get('preventability_score', 0)}%"
            )

            skill_gaps = analysis.get("skill_gap_analysis", {}).get(
                "identified_gaps", []
            )
            if skill_gaps:
            ...
            if training_opp:
                print(
                    f"   • Training Recommended: {training_opp.get('primary_focus', 'N/A')}"
                )
                print(
                    f"   • Impact Level: {training_opp.get('estimated_impact', 'N/A')}"
                )
                ...
                if analysis.get("training_recommendation"):
                    ...
                    print(
                        f"   • AI Training Module Generated: {training_content.get('title', 'N/A')}"
                    )
                    print(
                        f"   • Duration: {training_content.get('estimated_duration_minutes', 0)} minutes"
                    )
            else:
...
async def run_performance_analytics():
    ...
    if jake_metrics.get("error"):
        ...
        print(
            f"   • Efficiency Improvement: {improvements.get('efficiency_improvement_percent', 0)}%"
        )
        print(
            f"   • First-Time Fix Improvement: {improvements.get('first_time_fix_improvement_percent', 0)}%"
        )
        print(
            f"   • Work Order Volume Change: {improvements.get('work_order_volume_change', 0)}"
        )
...
async def run_dashboard_integration():
    ...
    print(
        f"   • Training Modules Generated: {overview.get('training_modules_generated', 0)}"
    )

    business_intelligence = dashboard_data.get("business_intelligence", {})
    print(
        f"   • Estimated Annual Savings: {business_intelligence.get('estimated_annual_savings', 'N/A')}"
    )
    print(
        f"   • Repeat Failure Reduction: {business_intelligence.get('repeat_failure_reduction', 'N/A')}"
    )
    print(
        f"   • First-Time Fix Improvement: {business_intelligence.get('first_time_fix_improvement', 'N/A')}"
    )
