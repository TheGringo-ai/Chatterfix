...
class MobileIntegrationTester:
    ...
    def test_backend_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.log(
                "PASS",
                f"Backend accessible - Status: {response.status_code}, Time: {result['response_time_ms']:.0f}ms",
            )
            return result

        except requests.exceptions.RequestException as e:
            self.log("FAIL", f"Backend not accessible: {str(e)}")
            return {"status": "FAIL", "error": str(e), "accessible": False}
    ...
    def test_auth_configuration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log(
                    "PASS",
                    f"Auth config retrieved: Firebase={config.get('use_firebase', False)}",
                )
                return {
                    "status": "PASS",
                    "config": config,
    ...
    def test_authentication_flow(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code in [302, 200]:
                ...
                self.log(
                    "PASS", f"Authentication successful - Token: {bool(session_token)}"
                )
                return {
                    "status": "PASS",
            ...
            try:
                ...
                if response.status_code == 200:
                    try:
                        ...
                        self.log(
                            "PASS",
                            f"{description}: ✓ (size: {len(response.content)} bytes)",
                        )
                ...
                if response.status_code == 200:
                    try:
                        ...
                        results[endpoint] = {
                            ...
                            "data_keys": (
                                list(data.keys()) if isinstance(data, dict) else "array"
                            ),
                        }
    ...
    def test_cors_headers(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.session.options(
                f"{self.base_url}/auth/config", headers=headers
            )
            ...
            mobile_friendly = (
                ...
                in ["*", "https://chatterfix-mobile.app"]
                or "chatterfix"
                in str(cors_headers.get("access-control-allow-origin", "")).lower()
            )
