...
class SalesDemoTester:
            ...
            try:
                if method == "GET":
                    response = self.session.get(
                        f"{self.local_url}{endpoint}", timeout=5
                    )
                else:
    ...
    def test_mobile_dependencies(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"dependencies_ready": False, "error": str(e), "can_build": False}
    ...
    def test_ci_cd_configuration(self) -> Dict[str, Any]:
        ...
        ci_files = [
            ...
            (
                "/Users/fredtaylor/ChatterFix/.github/workflows/deploy-simple.yml",
                "GitHub Actions",
            ),
        ]
    ...
    def test_integration_points(self) -> Dict[str, Any]:
        ...
        try:
            response = self.session.get(
                f"{self.local_url}/analytics/kpi/summary?days=30"
            )
            data = response.json()
    ...
    def generate_demo_readiness_report(self) -> Dict[str, Any]:
        ...
        report["overall_demo_readiness"] = {
            ...
            "status": (
                "🟢 READY"
                if demo_ready_score >= 3
                else "🟡 NEEDS WORK" if demo_ready_score >= 2 else "🔴 NOT READY"
            ),
    ...
    def print_demo_summary(self, report: Dict[str, Any]):
        ...
        self.log(
            "INFO", f"Readiness Score: {readiness['score']} ({readiness['percentage']})"
        )
        ...
        self.log(
            "INFO",
            f"Backend APIs: {'✅' if report['tests']['backend']['critical_for_demo'] else '❌'}",
        )
        self.log(
            "INFO",
            f"Mobile Structure: {'✅' if report['tests']['mobile_structure']['demo_ready'] else '❌'}",
        )
        self.log(
            "INFO",
            f"Dependencies: {'✅' if report['tests']['dependencies']['can_build'] else '❌'}",
        )
        self.log(
            "INFO",
            f"CI/CD Pipeline: {'✅' if report['tests']['ci_cd']['deployment_ready'] else '❌'}",
        )
        self.log(
            "INFO",
            f"Integration: {'✅' if report['tests']['integration']['demo_ready'] else '❌'}",
        )
