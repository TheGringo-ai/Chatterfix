...
class TestTrainingEndpoints:
    ...
    def test_get_modules(self, mock_get_manager, mock_firestore_manager):
        ...
        mock_firestore_manager.get_collection.return_value = [modules[0] for modules in [mock_firestore_manager.get_collection.return_value] if modules[0]["skill_category"] == "safety"]
    ...
    def test_start_training(self, mock_get_manager, mock_firestore_manager):
        ...
        response = client.post(
            "/training/modules/module_1/start",
            data={"user_id": "1"}
        )
    ...
    def test_assign_training(self, mock_get_manager, mock_firestore_manager):
        ...
        response = client.post(
            "/training/assign",
            data={"user_id": "2", "module_id": "module_1"}
        )
    ...
    @patch("app.routers.training.get_firestore_manager")
    def test_training_center_authenticated(self, mock_get_manager, mock_firestore_manager, valid_session_headers):
...
class TestTrainingHelperFunctions:
    ...
    @patch("app.routers.training.get_firestore_manager")
    @pytest.mark.asyncio
    async def test_get_user_training_with_modules(self, mock_get_manager, mock_firestore_manager):
    ...
    @patch("app.routers.training.get_firestore_manager")
    @pytest.mark.asyncio
    async def test_get_user_training_stats(self, mock_get_manager, mock_firestore_manager):
...
class TestTrainingGenerator:
    ...
    @pytest.mark.asyncio
    async def test_generate_from_manual(self, mock_get_manager, mock_genai, mock_firestore_manager):
        ...
        mock_response.text = json.dumps({
            ...
            "sections": [
                {"title": "Overview", "content": "Training overview"}
            ],
            "quiz": [
                {
        ...
        mock_get_manager.return_value = mock_firestore_manager
        mock_firestore_manager.create_training_module.return_value = "generated_module_id"
